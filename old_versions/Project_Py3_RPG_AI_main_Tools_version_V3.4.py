    ...
    def __init__(self, storage: StorageManager):
        ...
        self.default_prompts = {
            ...
            "stage1_random_event": (
                ...
                "Твоя задача:\n"
                "1. Вызови roll_dice(dice_type='d100') и определи, произошло ли событие.\n"
                "2. Если событие произошло, вызови roll_dice(dice_type='d20') для качества.\n"
                "3. Опиши событие (1 предложение, на русском).\n"
                "4. Вызови report_random_event с параметрами:\n"
                "   - dice_value: значение d20 (если событие было) или d100 (если не было)\n"
                "   - event_occurred: true/false\n"
                "   - description: описание события (пустая строка, если не произошло)\n\n"
                "Не пиши лишнего текста, только вызовы инструментов и описание в report_random_event."
            ),
            "stage1_turn_order": (
                "Ты — рассказчик.\n\n"
...
class MainApp(tk.Tk):
    ...
    def _after_stage1_random_event(self, tool_calls, content, extra=None):
        ...
        # Если не нашли, повторяем
        self.center_panel.display_message("⚠️ Модель не вызвала report_random_event. Повтор.\n", "error")
        self.update("stage1_random_event")
    ...
    def _stage3_final(self, _=None):
        """Этап 6: Финальное повествование с бросками d20 для каждого NPC."""
        self._log_debug("=== STAGE3: final narration ===")
        ...
        npc_actions_text = ""
        for npc_id in self.stage_data.get("turn_order", []):
            npc = self.characters.get(npc_id)
        ...
        prompt_template = self.prompt_manager.get_prompt_content("stage3_final")
        main_prompt = prompt_template.format(
        ...
        # Запускаем временный вывод (стрим), модель будет вызывать roll_dice и finish_stage
        self.center_panel.start_temp_response()
        # Ожидаем вызовов инструментов, включая finish_stage
        self._send_model_request(messages, self._after_stage3_final, stage_name="stage3_final", use_temp=True, expect_tool_calls=True)
    ...
    def _send_model_request(self, messages: List[Dict], callback, extra=None, expect_tool_calls=True, stage_name: str = None, use_temp: bool = False):
        """
        ...
        def _chat_loop(current_messages, depth=0):
            ...
            def stream_and_process():
                ...
                error = None
                try:
                    for chunk in self.lm_client.chat_completion_stream(messages=current_messages, model=model, temperature=temp, max_tokens=max_tok):
                ...
                for tc in tool_calls:
                    ...
                    if name == "roll_dice":
                        ...
                        try:
                            ...
                            current_messages.append(tool_response)
                            # Добавляем явное текстовое сообщение, чтобы модель точно увидела описания
                ...
                if handled_calls:
                    # Добавляем сообщение ассистента с tool_calls (если ещё не добавлено)
...
class LeftPanel(ttk.Frame):
    ...
    def refresh_session_list(self):
        ...
        for sid in sessions:
            ...
            name = data.get("name", "Без имени") if data else "Ошибка"
    ...
    def get_session_name(self, session_id: str) -> str:
        ...
        return data.get("name", "Без имени") if data else "?"
