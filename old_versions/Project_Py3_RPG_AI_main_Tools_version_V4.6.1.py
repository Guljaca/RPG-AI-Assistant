    ...
    def __init__(self, storage: StorageManager):
        ...
        self.default_prompts = {
            ...
            "stage1_request_descriptions": (
                ...
                "4. ВНИМАНИЕ, КРАЙНЕ ВАЖНО, ЕСЛИ НЕ ВЫПОЛНИШЬ, ВСЕ СЛОМАЕТСЯ: Если событие устраивает: вызови confirm_scene с location_id, character_ids, item_ids"
                
            ),
            "stage1_validate_scene": (
                "Ты — проверяющий рассказчик. Твоя задача — оценить, корректна ли сцена, предложенная другим рассказчиком.\n\n"
                "Сцена, которую нужно проверить:\n{scene_summary}\n\n"
                "Описания объектов:\n{descriptions}\n\n"
                "Исходное сообщение игрока:\n{user_message}\n\n"
                "Критерии корректности:\n"
                "- Локация должна существовать и соответствовать описанию.\n"
                "- Все указанные персонажи и предметы должны присутствовать в описаниях объектов.\n"
                "- Сцена должна быть логичной и не противоречить известным фактам.\n"
                "- Сцена должна соответствовать сообщению игрока (не игнорировать его действие).\n\n"
                "Если сцена корректна, вызови report_validation_result(valid=true).\n"
                "Если сцена некорректна, вызови report_validation_result(valid=false, feedback='Причина отказа').\n"
            ...
            "stage1_validate_random_event": (
                "Ты — проверяющий рассказчик. Оцени корректность сгенерированного случайного события.\n\n"
                "Контекст сцены:\n{scene_info}\n\n"
                "Описания объектов:\n{descriptions}\n\n"
                "Действие игрока:\n{player_action}\n\n"
                "Сгенерированное событие:\n{event_description}\n\n"
                "Критерии корректности:\n"
                "- Событие логично вытекает из текущей ситуации.\n"
                "- Не противоречит описаниям объектов и известным фактам.\n"
                "- Соответствует жанру и стилю игры.\n"
                "- Не нарушает игровую механику (например, не даёт нечестного преимущества без броска).\n\n"
                "Если событие корректно, вызови report_validation_result(valid=true).\n"
                "Если некорректно, вызови report_validation_result(valid=false, feedback='Причина отказа').\n"
                "Не добавляй лишний текст, только вызов функции."
            )
...
class MainApp(tk.Tk):
    def __init__(self):
        ...
        self.event_handlers = {
            ...
            "stage1_validate_scene": self._stage1_validate_scene,
            "stage1_validate_random_event": self._stage1_validate_random_event,
        }
    ...
    def _handle_update_object(self, obj_type: str, data: dict):
        """
        Универсальный обработчик создания/обновления объектов.
        obj_type: 'narrators', 'characters', 'locations', 'items'
        data: dict с ключами id (опционально), name, description, is_player (только для characters)
        """
        obj_id = data.get("id")
        ...
        # Получаем словарь объектов (self.narrators, self.characters и т.д.)
        ...
        # Определяем класс для создания нового объекта
        cls_map = {
            "narrators": Narrator,
    ...
    def _stage1_validate_random_event(self, data=None, retry_count=0):
        if isinstance(data, dict):
            retry_count = data.get("retry_count", 0)
        self._log_debug(f"=== STAGE1: validate_random_event (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"🔍 Этап 6/10: Валидация случайного события (попытка {retry_count+1})...\n")

        # Если события нет – валидировать нечего, сразу на следующий этап
        if not self.stage_data.get("event_occurred", False):
            self.center_panel.display_system_message("✅ Событие не произошло, валидация не требуется.\n")
            self.update("stage1_turn_order")
            return

        descriptions_text = "\n".join([f"{oid}: {desc}" for oid, desc in self.stage_data["descriptions"].items()])
        scene_info = (
            f"Локация: {self.stage_data.get('scene_location_id', 'не определена')}\n"
            f"Персонажи: {', '.join(self.stage_data.get('scene_character_ids', []))}\n"
            f"Предметы: {', '.join(self.stage_data.get('scene_item_ids', []))}"
        )
        player_action = self.stage_data.get("player_action_desc", "нет действия")
        event_desc = self.stage_data.get("event_desc", "")

        prompt_template = self.prompt_manager.get_prompt_content("stage1_validate_random_event")
        # Если промт не найден, используем встроенный (можно также загрузить из default_prompts)
        ...
        system_messages = self._build_context_messages(
            stage_name="stage1_validate_random_event",
            main_prompt=main_prompt
        )
        messages = [
            {"role": "user", "content": f"Проверь корректность события: {event_desc}"}
        ] + system_messages

        self._send_model_request(messages, self._after_stage1_validate_random_event,
                                extra={"retry_count": retry_count},
                                stage_name="stage1_validate_random_event")

    def _after_stage1_validate_random_event(self, tool_calls, content, extra=None):
        retry_count = extra.get("retry_count", 0) if extra else 0
        self._log_debug("AFTER stage1_validate_random_event", f"tool_calls: {tool_calls}")
        ...
        if not report_call:
            if retry_count < 2:
                self.center_panel.display_message(f"⚠️ Модель не вызвала report_validation_result. Повтор валидации ({retry_count+1}/2)...\n", "error")
                self.update("stage1_validate_random_event", {"retry_count": retry_count+1})
                return
            else:
                self.center_panel.display_message("❌ Модель не вызвала report_validation_result после 3 попыток. Событие считается невалидным.\n", "error")
                self._retry_random_event_generation()
                return
        ...
        try:
            ...
            if valid:
                self.center_panel.display_system_message("✅ Случайное событие успешно прошло валидацию.\n")
                self.update("stage1_turn_order")
            else:
                self.center_panel.display_system_message(f"❌ Событие не прошло валидацию. Причина: {feedback}\n")
                self._retry_random_event_generation()
        except Exception as e:
            self._log_debug("ERROR", f"report_validation_result parse error: {e}")
            if retry_count < 2:
                self.center_panel.display_message(f"⚠️ Ошибка обработки report_validation_result: {e}. Повтор валидации ({retry_count+1}/2)...\n", "error")
                self.update("stage1_validate_random_event", {"retry_count": retry_count+1})
            else:
                self.center_panel.display_message(f"❌ Ошибка после 3 попыток: {e}. Событие считается невалидным.\n", "error")
                self._retry_random_event_generation()

    def _retry_random_event_generation(self):
        """Повторяет генерацию случайного события, если валидация не пройдена, не более 2 раз."""
        retry_key = "random_event_retries"
        if retry_key not in self.stage_data:
            self.stage_data[retry_key] = 0
        current_retry = self.stage_data[retry_key]
        if current_retry < 2:
            self.stage_data[retry_key] = current_retry + 1
            self.center_panel.display_system_message(f"🔄 Повторная генерация случайного события (попытка {current_retry+1}/2)...\n")
            self.update("stage1_random_event", {"retry_count": current_retry+1})
        else:
            self.center_panel.display_system_message("⚠️ Событие не прошло валидацию после 3 попыток. Пропускаем событие.\n", "error")
            # Сбрасываем счётчик для следующего раза
            self.stage_data.pop(retry_key, None)
    ...
    def _stage1_request_descriptions(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"🔍 Этап 1/10: Определение объектов сцены (попытка {retry_count+1})...\n")

        # При повторной попытке сбрасываем данные сцены, но оставляем исходное сообщение
        if retry_count > 0:
    ...
    def _after_stage1_descriptions(self, tool_calls, content, extra=None):
        ...
        if confirm_call:
            try:
                ...
                self.center_panel.display_system_message(f"✅ Сцена сгенерирована моделью:\n{summary}\n")

                # Запрашиваем описания объектов, если их ещё нет
                ...
                # Переходим к этапу валидации
                self.update("stage1_validate_scene", {"retry_count": 0})
                return
            except Exception as e:
                self._log_debug("ERROR", f"confirm_scene parse error: {e}")
                if retry_count < 2:
                    self.center_panel.display_message(f"⚠️ Техническая ошибка при обработке confirm_scene: {e}. Повтор ({retry_count+1}/2)...\n", "error")
                    self.update("stage1_request_descriptions", {"retry_count": retry_count+1})
                else:
                    self.center_panel.display_message(f"❌ Критическая ошибка после 3 попыток: {e}. Генерация прервана.\n", "error")
                    self.is_generating = False
                    self.center_panel.set_input_state(tk.NORMAL)
                return

        # Если confirm_scene не вызван
        if self._is_model_rejection(content):
            self.center_panel.display_system_message(
        ...
        if retry_count < 2:
            ...
            self.center_panel.display_message(
                "❌ Модель не вызвала confirm_scene после 3 попыток. Генерация прервана.\n",
                "error"
            )
            self.is_generating = False
            self.center_panel.set_input_state(tk.NORMAL)

    def _stage1_validate_scene(self, data=None, retry_count=0):
        if isinstance(data, dict):
            retry_count = data.get("retry_count", 0)
        self._log_debug(f"=== STAGE1: validate_scene (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"🔍 Этап 2/10: Валидация сцены (попытка {retry_count+1})...\n")

        descriptions_text = "\n".join([f"{oid}: {desc}" for oid, desc in self.stage_data["descriptions"].items()])
        prompt_template = self.prompt_manager.get_prompt_content("stage1_validate_scene")
        main_prompt = prompt_template.format(
        ...
        self._send_model_request(messages, self._after_stage1_validate_scene, extra={"retry_count": retry_count}, stage_name="stage1_validate_scene")

    def _after_stage1_validate_scene(self, tool_calls, content, extra=None):
        retry_count = extra.get("retry_count", 0) if extra else 0
        self._log_debug("AFTER stage1_validate_scene", f"tool_calls: {tool_calls}")
        ...
        if not report_call:
            if retry_count < 2:
                self.center_panel.display_message(f"⚠️ Модель не вызвала report_validation_result. Повтор ({retry_count+1}/2)...\n", "error")
                self.update("stage1_validate_scene", {"retry_count": retry_count+1})
                return
            else:
                self.center_panel.display_message("❌ Модель не вызвала report_validation_result после 3 попыток. Сцена считается невалидной.\n", "error")
                # Переходим к повторной генерации сцены
        ...
        try:
            ...
            feedback = args.get("feedback", "")
            if valid:
                self.center_panel.display_system_message("✅ Сцена успешно прошла валидацию.\n")
                ...
                self.center_panel.display_system_message(f"❌ Сцена не прошла валидацию. Причина: {feedback}\n")
                self._retry_scene_generation()
        except Exception as e:
            self._log_debug("ERROR", f"report_validation_result parse error: {e}")
            if retry_count < 2:
                self.center_panel.display_message(f"⚠️ Ошибка обработки report_validation_result: {e}. Повтор ({retry_count+1}/2)...\n", "error")
                self.update("stage1_validate_scene", {"retry_count": retry_count+1})
            else:
                self.center_panel.display_message(f"❌ Ошибка после 3 попыток: {e}. Сцена считается невалидной.\n", "error")
                self._retry_scene_generation()

    def _retry_scene_generation(self):
        """Повторяет генерацию сцены, если валидация не пройдена, не более 2 раз."""
        ...
        if current_retry < 2:
            self.stage_data[retry_key] = current_retry + 1
            self.center_panel.display_system_message(f"🔄 Повторная генерация сцены (попытка {current_retry+1}/2)...\n")
            self.update("stage1_request_descriptions", {"retry_count": current_retry+1})
        else:
            self.center_panel.display_system_message("❌ Сцена не прошла валидацию после 3 попыток. Генерация прервана.\n", "error")
            self.is_generating = False
            self.center_panel.set_input_state(tk.NORMAL)
            # Сбрасываем счётчик для следующего раза
            self.stage_data.pop(retry_key, None)
    ...
    def _is_model_rejection(self, text: str) -> bool:
        """Проверяет, содержит ли текст ответа модели явный отказ подтвердить сцену."""
        if not text:
            return False
        rejection_phrases = [
    ...
    def _stage1_truth_check(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"🔍 Этап 3/10: Проверка правдивости сообщения (попытка {retry_count+1})...\n")
    ...
    def _stage1_player_action(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"🎲 Этап 4/10: Обработка действия игрока (попытка {retry_count+1})...\n")
    ...
    def _stage1_random_event(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"🎲 Этап 5/10: Генерация случайного события (попытка {retry_count+1})...\n")
        ...
        # Передаём в callback номер попытки, чтобы при необходимости повторить
        self._send_model_request(messages, self._after_stage1_random_event,
                                extra={"retry_count": retry_count},
                                stage_name="stage1_random_event")
    ...
    def _after_stage1_random_event(self, tool_calls, content, extra=None):
        ...
        # Обработка случая, когда модель не вызвала функцию (fallback)
        if not report_call:
            if self._try_extract_event_from_text(content):
                # удалось извлечь из текста – сохраняем и переходим к валидации
                self.center_panel.display_system_message("✅ Случайное событие извлечено из текста.\n")
                self.update("stage1_validate_random_event", {"retry_count": 0})
                return
            if retry_count < 2:
                ...
                self.center_panel.display_message("❌ Модель не вызвала report_random_event после 3 попыток. Событие считается отсутствующим.\n", "error")
                self.stage_data["event_occurred"] = False
                self.stage_data["event_desc"] = ""
        ...
        try:
            ...
            if event_occurred:
                self.center_panel.display_system_message(f"✨ Сгенерировано событие: {event_desc[:100]}...\n")
            else:
                self.center_panel.display_system_message("✅ Случайное событие не произошло.\n")

            # Переходим к валидации (сбрасываем счётчик повторных попыток генерации)
            self.update("stage1_validate_random_event", {"retry_count": 0})
        except Exception as e:
            ...
            if retry_count < 2:
                ...
                self.center_panel.display_message(f"❌ Ошибка после 3 попыток: {e}. Событие считается отсутствующим.\n", "error")
                self.stage_data["event_occurred"] = False
                self.stage_data["event_desc"] = ""
                self.update("stage1_validate_random_event", {"retry_count": 0})

    def _try_extract_event_from_text(self, content: str) -> bool:
        """Пытается извлечь данные о событии из текстового ответа модели, если не был вызван report_random_event."""
        if not content:
            return False
        import re
    ...
    def _stage1_turn_order(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"📋 Этап 6/10: Определение порядка NPC (попытка {retry_count+1})...\n")
    ...
    def _stage2_process_npc(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"🎭 Этап 7/10: Обработка NPC (попытка {retry_count+1})...\n")
    ...
    def _stage3_final(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"📖 Этап 9/10: Генерация финального ответа (попытка {retry_count+1})...\n")
    ...
    def _stage4_summary(self, data=None, retry_count=0):
        ...
        self.center_panel.display_system_message(f"📝 Этап 10/10: Сохранение краткой памяти (попытка {retry_count+1})...\n")
        ...
        # Улучшенный промт для генерации полного, осмысленного резюме
        prompt = (
            ...
            "Не пиши ничего лишнего, только факт. Не обрывай предложение на середине.\n\n"
        ...
        self._send_model_request(
            ...
            use_temp=True,
            # Увеличиваем токены для генерации памяти
            tools_override=[]
        )
    ...
    def _after_stage4_summary(self, tool_calls, content, extra=None):
        ...
        # Проверяем, что резюме не пустое и не слишком короткое (обрыв)
        if not summary or len(summary) < 10:
            if retry_count < 2:
                self.center_panel.display_message("⚠️ Краткая память слишком короткая или пустая. Повтор...\n", "error")
                self.update("stage4_summary", {"retry_count": retry_count+1})
                return
            else:
                summary = "Игрок продолжил свои действия."

        # Добавляем в список памяти
    ...
    def _build_context_messages(self, stage_name: str, main_prompt: str = "", extra_prompts: List[str] = None) -> List[Dict[str, str]]:
        ...
        # 1. Краткая память (самая старая информация, но важная для контекста)
        ...
        # 2. Загружаем конфигурацию этапа из stage_prompts_config
        config = self.stage_prompts_config.get(stage_name, [])
        
        # 3. Добавляем все элементы из конфигурации (кроме history, если он есть, обработаем позже)
        history_entry = None
        for entry in config:
            if entry.startswith("history"):
                history_entry = entry
                continue  # отложим добавление истории
        ...
        # 4. Добавляем историю чата (user/assistant), если она не была явно запрещена
        #    Если в конфиге есть history:0 – не добавляем историю.
        #    Если history:N – добавляем последние N сообщений.
        #    Если history без числа – добавляем все.
        #    Если history вообще нет в конфиге – добавляем все сообщения (по умолчанию).
        add_history = True
        history_limit = None
        if history_entry:
            ...
            if len(parts) > 1:
                ...
                if limit_str.isdigit():
                    ...
                    history_limit = None  # все сообщения
            else:
                history_limit = None  # все сообщения
        else:
            # Если в конфиге нет history, добавляем историю по умолчанию (все сообщения)
            history_limit = None
    ...
    def _handle_delete_object(self, obj_type: str, obj_id: str):
        ...
        # Удаляем из активного профиля, если есть
        ...
        self._refresh_all_ui()
        self._save_current_session_safe()
        # Очищаем рассказчиков из конфигурации этапов (только для narrators)
        if obj_type == "narrators":
...
class StagePromptsTab(ttk.Frame):
    ...
    def __init__(self, parent, app: MainApp):
        super().__init__(parent)
        self.app = app
        # Актуальный список этапов (должен совпадать с MainApp.stage_names)
        self.stages = [
            "stage1_request_descriptions",
            "stage1_validate_scene",
            "stage1_truth_check",
            "stage1_player_action",
            "stage1_random_event",
            "stage1_validate_random_event",   # <-- новая стадия
            "stage1_turn_order",
            "stage2_npc_action",
            "stage3_final",
            "stage4_summary"
