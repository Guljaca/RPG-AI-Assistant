...
class MainApp(tk.Tk):
    ...
    def _after_stage1_descriptions(self, tool_calls, content, extra=None):
        ...
        # Случай 1: модель вызвала confirm_scene → всё штатно
        if confirm_call:
            try:
                ...
                # Ошибка парсинга считается технической ошибкой
                if retry_count < 2:
        ...
        # Случай 2: confirm_scene не вызван
        # Проверяем, не является ли это осознанным отказом модели
        if self._is_model_rejection(content):
            # Отказ модели – не ошибка, а логическое решение
            self.center_panel.display_system_message(
                f"⚠️ Модель отклонила сцену. Причина (из ответа модели):\n{content[:300]}\n"
                "Генерация остановлена, так как модель не сочла возможным продолжить.\n"
                "Попробуйте изменить действие игрока или состав доступных объектов.\n"
            )
            self.is_generating = False
            self.center_panel.set_input_state(tk.NORMAL)
            return

        # Случай 3: техническая ошибка – модель не вызвала confirm_scene и не отказалась явно
        if retry_count < 2:
            self.center_panel.display_message(
                f"⚠️ Модель не вызвала confirm_scene. Повторная попытка ({retry_count+1}/2)...\n"
                f"Ответ модели (техническая проблема): {content[:200] if content else 'пустой ответ'}\n",
                "error"
            )
            self.update("stage1_request_descriptions", {"retry_count": retry_count+1})
        else:
            self.center_panel.display_message(
                ...
                "Проверьте соединение с LM Studio или настройки модели.\n",
                "error"
            )
            self.is_generating = False
            self.center_panel.set_input_state(tk.NORMAL)
