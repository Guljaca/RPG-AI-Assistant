    ...
    def __init__(self, storage: StorageManager):
        ...
        self.default_prompts = {
            ...
            "stage1_request_descriptions": (
                ...
                "Доступные объекты в мире (только ID и названия):\n{available_objects}\n\n"
                "Твоя задача — определить, какие объекты (локация, персонажи, предметы) реально участвуют в сцене.\n"
                "Для этого:\n"
                "1. Выбери **кандидатов** — тех, кто потенциально может быть в этой сцене (по названию и логике).\n"
                "2. Вызови `send_object_info` с массивом `object_ids` этих кандидатов, чтобы получить их полные описания.\n"
                "3. После получения описаний (они придут в следующем сообщении от системы) **сначала напиши свои рассуждения** в текстовом виде.\n"
                "   - какие объекты подходят и почему,\n"
                "   - какие не подходят и почему (например, монстр не может быть в доме).\n"
                "4. Затем вызови `confirm_scene` с итоговыми:\n"
                "   - `location_id` (одна локация, может быть null, если не определена)\n"
                "   - `character_ids` (только подходящие)\n"
                "   - `item_ids` (только подходящие)\n\n"
                "Не вызывай confirm_scene без предварительного текстового анализа."
            ),
            "stage1_player_action": (
                ...
                "Правила бросков d20:\n{dice_rules}\n\n"
                "Твоя задача:\n"
                "1. Вызови roll_dice(dice_type='d20'), чтобы получить значение броска.\n"
                "2. На основе значения определи результат (крит.провал, провал, успех, большой успех, крит.успех).\n"
                "3. Опиши, что получилось у игрока (1-2 предложения, на русском, без упоминания броска).\n"
                "4. Вызови report_player_action с параметрами dice_value и description.\n"
                "Не пиши лишнего текста, только вызовы инструментов и описание в report_player_action."
            ),
            "stage1_random_event": (
                "Ты — рассказчик в TTRPG.\n\n"
                "Описания объектов:\n{descriptions}\n\n"
                "Действие игрока: {player_action}\n\n"
                "Правила определения случайного события:\n"
                "- Бросок d100: если значение от 1 до 30 включительно — событие происходит.\n"
                "- Если значение от 31 до 100 — событие не происходит.\n\n"
                "Если событие произошло, дополнительно брось d20 для определения качества:\n"
                "  1-4: негативное событие\n"
                "  5-15: нейтральное событие\n"
                "  16-20: позитивное событие\n\n"
                "Твоя задача — выполнить строго по шагам:\n\n"
                "ШАГ 1. Вызови roll_dice(dice_type='d100') и дождись результата.\n\n"
                "ШАГ 2. Если результат roll_dice (d100) <= 30:\n"
                "   - Вызови roll_dice(dice_type='d20') для определения качества.\n"
                "   - Придумай ОДНО предложение на русском, описывающее событие.\n"
                "   - Вызови report_random_event с параметрами:\n"
                "       dice_value = (значение d20)\n"
                "       event_occurred = true\n"
                "       description = \"твоё описание\"\n\n"
                "ШАГ 3. Если результат roll_dice (d100) > 30:\n"
                "   - Вызови report_random_event с параметрами:\n"
                "       dice_value = (значение d100)\n"
                "       event_occurred = false\n"
                "       description = \"\"\n\n"
                "ВАЖНО: Ты ОБЯЗАН вызвать report_random_event в любом случае. Не пиши никакой дополнительный текст, только вызовы инструментов и описание в report_random_event.\n\n"
                "ПРИМЕР правильного вызова, когда событие НЕ произошло (d100 = 67):\n"
                "report_random_event(dice_value=67, event_occurred=false, description=\"\")\n\n"
                "ПРИМЕР когда событие произошло (d100 = 12, d20 = 18):\n"
                "report_random_event(dice_value=18, event_occurred=true, description=\"Ветер внезапно хлопнул дверью, и с полки упала старая фотография.\")"
            ),
            "stage1_turn_order": (
                "Ты — рассказчик.\n\n"
                "Информация о текущей сцене:\n{scene_info}\n\n"
                "Список NPC на сцене:\n{npcs_list}\n\n"
                "Твоя задача: определить порядок ходов этих NPC (кроме игрока).\n"
                "Учти особенности локации, произошедшее событие, успех/провал действия игрока и предыдущие действия NPC.\n\n"
                "ВАЖНО: НЕ вызывай roll_dice, НЕ бросай кубики. Просто оцени ситуацию и реши, кто действует первым, кто вторым и т.д.\n"
                "Вызови report_turn_order с параметром character_ids (массив ID NPC в порядке очереди).\n"
                "Пример: report_turn_order(character_ids=['c2', 'c3'])\n"
                "Не пиши лишнего текста, только вызов инструмента. Никаких бросков кубиков!"
            ),
            "stage2_npc_action": (
            ...
            "stage3_final": (
                "Ты — рассказчик в TTRPG. Твоя задача — описать результат действий игрока и NPC, используя броски d20.\n\n"
                "Локация и описание окружения:\n{location_desc}\n\n"
                "Действие игрока (успех/провал):\n{player_action_outcome}\n\n"
                "Случайное событие:\n{event_description}\n\n"
                "Планируемые действия NPC (это только их НАМЕРЕНИЯ, а не фактические события):\n{npcs_actions}\n\n"
                "Правила бросков d20:\n{dice_rules}\n\n"
                "ВАЖНО: НЕ ПЕРЕСКАЗЫВАЙ НАМЕРЕНИЯ NPC. Опиши, что на самом деле ПРОИЗОШЛО.\n"
                "Для каждого NPC выполни бросок d20 (вызови roll_dice(dice_type='d20')).\n"
                "Затем напиши связный рассказ от второго лица ('ты'), включая:\n"
                "  - Результат действия игрока (уже дан).\n"
                "  - Случайное событие (если есть).\n"
                "  - Что реально делают NPC: их движения, слова, звуки, последствия. Не пиши 'думает', 'намерен' – пиши 'вскакивает', 'спотыкается', 'кричит', 'замирает' и т.п.\n"
                "  - Как броски d20 повлияли на NPC (успех – план удался, провал – что-то помешало, крит – неожиданный поворот).\n"
                "Длина: 4-8 предложений. Не упоминай броски кубиков и ID объектов в ответе.\n\n"
                "После того как напишешь рассказ, ОБЯЗАТЕЛЬНО вызови finish_stage().\n"
                "ЗАПРЕЩЕНО вызывать report_player_action, report_random_event, report_turn_order, report_npc_action или любые другие функции, кроме roll_dice и finish_stage."
            ),
            ...
            "stage3_final_auto": (
                "Ты — рассказчик в TTRPG. Твоя задача — описать результат действий игрока и NPC, используя готовые броски d20.\n\n"
                "Локация и описание окружения:\n{location_desc}\n\n"
                ...
                "- После того как напишешь рассказ, ОБЯЗАТЕЛЬНО вызови finish_stage() без аргументов.\n"
                "  Если ты не вызовешь finish_stage, система сочтёт генерацию незавершённой и повторит этап.\n\n"
...
class MainApp(tk.Tk):
    ...
    def _stage1_request_descriptions(self, _=None):
        """Этап 1: модель запрашивает описания кандидатов, затем фильтрует и вызывает confirm_scene."""
        self._log_debug("=== STAGE1: request_descriptions (with filtering) ===")
        self.center_panel.display_system_message("🔍 Этап 1/5: Определение объектов сцены (с фильтрацией)...\n")

        # Формируем список всех доступных объектов (ID + имя)
        objects_text = []
        ...
        # Первое сообщение от пользователя, чтобы модель поняла контекст
        messages = [
            {"role": "user", "content": f"Сообщение игрока: {self.stage_data['user_message']}"}
        ] + system_messages

        self._send_model_request(messages, self._after_stage1_descriptions, extra=None, stage_name="stage1_request_descriptions")

    def _after_stage1_descriptions(self, tool_calls, content, extra=None):
        """Обрабатывает только confirm_scene. send_object_info уже обработан в цикле."""
        self._log_debug("AFTER stage1_descriptions", f"tool_calls: {tool_calls}\ncontent: {content[:500] if content else ''}")
        if not tool_calls:
            self.center_panel.display_message("⚠️ Модель не вызвала confirm_scene. Повтор этапа.\n", "error")
            self.update("stage1_request_descriptions")
            return
        ...
        for tc in tool_calls:
            if tc["function"]["name"] == "confirm_scene":
                try:
                    ...
                    # Убедимся, что описания всех финальных объектов сохранены
                    ...
                    if self.stage_data["user_message"].startswith("Начнем игру") or self.stage_data["user_message"].startswith("SYSTEM:"):
                    ...
                    self.center_panel.display_message(f"⚠️ Ошибка при обработке confirm_scene: {e}\n", "error")
                    self.update("stage1_request_descriptions")
                    return
            elif tc["function"]["name"] == "send_object_info":
                # Игнорируем, так как уже обработано в цикле
                self._log_debug("send_object_info ignored in callback")
                return

        # Если дошли сюда – confirm_scene не найден
        self.center_panel.display_message("⚠️ Модель не вызвала confirm_scene. Повтор этапа.\n", "error")
        self.update("stage1_request_descriptions")

    # ---------- Этап 2: Действие игрока и бросок d20 ----------
    def _stage1_player_action(self, _=None):
        self._log_debug("=== STAGE1: player_action ===")
        self.center_panel.display_system_message("🎲 Этап 2/5: Обработка действия игрока...\n")
        ...
        messages = [
            {"role": "user", "content": f"Игрок хочет: {self.stage_data['user_message']}. Вызови roll_dice(dice_type='d20'), получи результат, опиши действие и вызови report_player_action."}
        ] + system_messages

        self._send_model_request(messages, self._after_stage1_player_action, expect_tool_calls=True, stage_name="stage1_player_action")
    ...
    def _after_stage1_player_action(self, tool_calls, content, extra=None):
        ...
        # Ищем вызов report_player_action
        for tc in tool_calls:
        ...
        # Если не нашли – повторяем этап
        self.center_panel.display_message("⚠️ Модель не вызвала report_player_action. Повтор.\n", "error")
        self.update("stage1_player_action")

    # ---------- Этап 3: Случайное событие ----------
    def _stage1_random_event(self, _=None):
        self._log_debug("=== STAGE1: random_event ===")
        self.center_panel.display_system_message("🎲 Этап 3/5: Проверка случайного события...\n")
        ...
        self._send_model_request(messages, self._after_stage1_random_event, stage_name="stage1_random_event")
    ...
    def _after_stage1_random_event(self, tool_calls, content, extra=None):
        ...
        # Если не нашли вызов инструмента, пытаемся извлечь данные из текстового ответа модели (fallback)
        if content and isinstance(content, str):
            import re
            # Ищем упоминание броска d100
            d100_match = re.search(r'd100[^\d]*(\d{1,3})', content, re.IGNORECASE)
            # Ищем фразы "не произошло", "не случилось", "ничего не произошло"
            ...
            if d100_match:
                ...
                # Если число >30 и нет признаков события, считаем, что события не было
                if dice_val > 30 and not event_occurred:
                    ...
                    self.update("stage1_turn_order")
                    return
                # Иначе пытаемся найти описание события
                desc_match = re.search(r'(?:описание|событие|произошло)[:：]\s*(.+?)(?=\n|$)', content, re.IGNORECASE)
                if desc_match:
                    event_desc = desc_match.group(1).strip()
                else:
                    # берём первое предложение после числа
                    sentences = re.split(r'[.!?]+', content)
                    event_desc = sentences[1].strip() if len(sentences) > 1 else "Произошло что-то неопределённое."
        ...
        # Если ничего не сработало — повторяем этап
        self.center_panel.display_message("⚠️ Модель не вызвала report_random_event и не удалось извлечь данные из текста. Повтор этапа.\n", "error")
        self.update("stage1_random_event")

    # ---------- Этап 4: Определение порядка ходов NPC ----------
    def _stage1_turn_order(self, _=None):
        self._log_debug("=== STAGE1: turn_order ===")
        self.center_panel.display_system_message("📋 Этап 4/5: Определение порядка NPC (моделью)...\n")
        ...
        # Формируем подробное описание сцены для модели
        descriptions_text = "\n".join([f"{oid}: {desc}" for oid, desc in self.stage_data["descriptions"].items()])
        scene_info = f"""
    ...
    """
        ...
        self._send_model_request(messages, self._after_stage1_turn_order, stage_name="stage1_turn_order")
    ...
    def _after_stage1_turn_order(self, tool_calls, content, extra=None):
        ...
        # fallback: берём всех NPC, которые есть в сцене, и сортируем по ID
        npc_ids = [cid for cid in self.stage_data.get("scene_character_ids", [])
                if not self.characters.get(cid, Character()).is_player]
        self.stage_data["turn_order"] = sorted(npc_ids)  # или любой другой порядок
    ...
    def _stage2_process_npc(self, _=None):
        ...
        self._log_debug(f"=== STAGE2: processing NPC {npc_id} ({npc.name}) ===")
        self.center_panel.display_system_message(f"🎭 Планирование действий NPC: {npc.name}...\n")
        ...
        # Ожидаем только текст, без вызова инструментов
    ...
    def _after_stage2_npc_action(self, tool_calls, content, extra):
        ...
        if content.strip():
            ...
            # Если модель ничего не вернула, ставим нейтральное описание
    ...
    def _stage3_final(self, _=None):
        """Этап 6: Финальное повествование с автоматическими бросками d20 для NPC."""
        self._log_debug("=== STAGE3: final narration (auto dice) ===")
        self.center_panel.display_system_message("📖 Этап 5/5: Генерация финального ответа...\n")
        
        location_id = self.stage_data.get("scene_location_id")
        if location_id:
            location_desc = self.stage_data["descriptions"].get(location_id, "Локация не описана")
        else:
            location_desc = "Локация не определена"
        
        # Формируем список планируемых действий NPC (то, что модель вернула на этапе stage2)
        npc_actions_text = ""
        ...
        is_game_start = (self.stage_data["user_message"].startswith("Начнем игру") or 
                        self.stage_data["user_message"].startswith("SYSTEM:"))
        if is_game_start:
        ...
        # ---- АВТОМАТИЧЕСКАЯ ГЕНЕРАЦИЯ БРОСКОВ ДЛЯ КАЖДОГО NPC ----
        dice_results = []
        for npc_id in npc_ids:
            npc = self.characters.get(npc_id)
            if not npc:
                continue
            dice_val = self.get_next_dice_value("d20")
            if dice_val is None:
                dice_val = random.randint(1, 20)
            # Интерпретация результата для подсказки модели (можно убрать, но полезно)
            if dice_val == 1:
                ...
                result_text = "большой успех"
            else:  # 20
                result_text = "критический успех"
            dice_results.append(f"{npc.name}: бросок d20 = {dice_val} → {result_text}")
            # Сохраняем бросок в stage_data для возможного использования в логах
            self.stage_data.setdefault("npc_dice", {})[npc_id] = dice_val
        
        dice_summary = "\n".join(dice_results) if dice_results else "Нет NPC, требующих бросков."
        
        # ---- ФОРМИРОВАНИЕ ПРОМТА БЕЗ ВЫЗОВОВ ROLL_DICE ----
        prompt_template = self.prompt_manager.get_prompt_content("stage3_final_auto")
        main_prompt = prompt_template.format(
            location_desc=location_desc,
            player_action_outcome=player_action_outcome,
            event_description=event_description,
            npcs_actions=npc_actions_text if npc_actions_text else "Никто ничего не делает.",
        ...
        messages = [
            {"role": "user", "content": f"Локация: {location_desc}\n\nДействие игрока: {player_action_outcome}\n\nСобытие: {event_description}\n\nПланируемые действия NPC:\n{npc_actions_text}\n\nРезультаты бросков для NPC:\n{dice_summary}"}
        ] + system_messages
        
        # Запускаем генерацию, ожидая finish_stage (инструменты оставляем как есть, но модель не будет вызывать roll_dice)
        self.center_panel.start_temp_response()
        self._send_model_request(messages, self._after_stage3_final, stage_name="stage3_final", use_temp=True, expect_tool_calls=True, tools_override=[])

    def _after_stage3_final(self, tool_calls, content, extra=None):
        """Обработка финального ответа: проверяем вызов finish_stage и завершаем генерацию."""
        self._log_debug("AFTER stage3_final", f"tool_calls: {tool_calls}\ncontent: {content[:500] if content else ''}")
        
        finish_called = any(tc["function"]["name"] == "finish_stage" for tc in tool_calls)
        
        # Если вызван не finish_stage и не roll_dice — повторяем этап
        unwanted_calls = [tc for tc in tool_calls if tc["function"]["name"] not in ("finish_stage", "roll_dice")]
        if unwanted_calls and not finish_called:
            self._log_debug("WARNING", f"Model called unwanted functions: {[tc['function']['name'] for tc in unwanted_calls]}. Repeating stage3_final.")
            self.center_panel.display_message("⚠️ Модель вызвала неверный инструмент. Повтор финального этапа.\n", "error")
            self.update("stage3_final")
            return
        
        final_text = content.strip()
        if not final_text and not finish_called:
            # Если нет текста и нет finish_stage, но есть roll_dice — возможно, модель пытается бросать кубики (запрещено)
            if any(tc["function"]["name"] == "roll_dice" for tc in tool_calls):
                self.center_panel.display_message("⚠️ Модель пыталась бросить кубики, но это запрещено на финальном этапе. Повтор.\n", "error")
                self.update("stage3_final")
                return
            self.center_panel.display_message("⚠️ Модель не сгенерировала ответ и не вызвала finish_stage. Повтор этапа.\n", "error")
            self.update("stage3_final")
            return
        
        # Если finish_stage не вызван, но текст есть — всё равно завершаем, но с предупреждением
        if not finish_called:
            self._log_debug("WARNING", "finish_stage not called by model, but finishing anyway.")
            self.center_panel.display_system_message("⚠️ Модель не вызвала finish_stage, но генерация завершена.\n")
        ...
        if final_text:
            ...
            self.center_panel.display_message("\nАссистент: (Рассказчик молчит)\n\n", "assistant")
            self.conversation_history.append({"role": "assistant", "content": "Рассказчик молчит."})
        ...
        self.center_panel.update_translation_button_state()
        self.current_debug_log_path = None
        self._log_debug("GENERATION_COMPLETED", final_text if final_text else "(empty)")
    ...
    def _build_context_messages(self, stage_name: str, main_prompt: str = "", extra_prompts: List[str] = None) -> List[Dict[str, str]]:
        """
        Собирает список сообщений (system, user, assistant) для передачи модели,
        следуя порядку, заданному в stage_prompts_config[stage_name].
        Каждый элемент конфигурации может быть:
        - простым именем промта (системное сообщение)
        - "narrator:ID" (системное сообщение со стилем рассказчика)
        - "history:N" (вставка N последних сообщений из conversation_history,
            где N=0 означает все сообщения)
        main_prompt и extra_prompts добавляются в конец (как system-сообщения).
        Если элемент конфигурации совпадает с stage_name, он пропускается,
        чтобы избежать дублирования с main_prompt.
        """
        messages = []
        config = self.stage_prompts_config.get(stage_name, [])
        for entry in config:
            if entry == stage_name:
                # Пропускаем, т.к. основной промт будет добавлен отдельно
                continue
