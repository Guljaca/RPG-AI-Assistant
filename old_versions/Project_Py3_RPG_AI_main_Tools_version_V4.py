    ...
    def __init__(self, storage: StorageManager):
        ...
        self.default_prompts = {
            ...
            "stage1_request_descriptions": (
                ...
                "3. Вызови confirm_scene с location_id, character_ids, item_ids.\n"
                "Не вызывай confirm_scene без предварительного анализа."
            ),
            "stage1_player_action": (
                ...
                "2. Опиши результат (1-2 предложения, на русском, без упоминания броска).\n"
                "3. Вызови report_player_action с параметрами dice_value и description."
            ),
            "stage1_random_event": (
                "Ты — рассказчик в TTRPG.\n\n"
                "Описания объектов:\n{descriptions}\n\n"
            ...
            "stage4_summary": (
                "Ты — помощник, который ведёт краткую историю игры для долговременной памяти.\n"
                "Из последнего обмена (действие игрока и ответ рассказчика) выдели **ОДНО ключественное изменение**, которое стоит запомнить:\n"
                "- Что изменилось в мире (дверь открыта/закрыта, предмет взят/потерян, что-то сломано/починено)?\n"
                "- Как NPC отреагировал эмоционально или действием, и **почему** (если причина важна для дальнейшего).\n"
                "- Успех или провал действия игрока (только если это влияет на дальнейший сюжет).\n"
                "Напиши **одно очень короткое предложение на русском (не более 10 слов)**. Не повторяй действие игрока дословно.\n"
                "Примеры хороших резюме:\n"
                "  Действие: «Я пытаюсь выбить дверь ногой». Ответ: «Дверь с треском открывается, но ты сломал замок». → Резюме: «Замок двери сломан».\n"
                "  Действие: «Я спрашиваю у стража, где казначейство». Ответ: «Страж хмурится и молча указывает налево». → Резюме: «Казначейство находится слева».\n"
                "  Действие: «Я бросаю камень в окно». Ответ: «Окно разбивается, из дома выбегает разъярённый пёс». → Резюме: «Разбито окно, выбежал злой пёс».\n"
                "  Действие: «Я толкаю сестру, чтобы разбудить». Ответ: «Кай просыпается, злится и кричит: \"Что за херня?!\"». → Резюме: «Сестра разозлилась из-за грубого пробуждения».\n"
                "Твоя очередь.\n"
                "Действие игрока: {user_message}\n"
                "Ответ рассказчика: {assistant_message}\n"
                "Краткая история (одно предложение, до 10 слов):"
            ),
...
class MainApp(tk.Tk):
    ...
    def _stage1_request_descriptions(self, _=None, retry_count=0):
        self._log_debug(f"=== STAGE1: request_descriptions (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"🔍 Этап 1/6: Определение объектов сцены (попытка {retry_count+1})...\n")
    ...
    def _stage1_player_action(self, _=None, retry_count=0):
        self._log_debug(f"=== STAGE1: player_action (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"🎲 Этап 2/6: Обработка действия игрока (попытка {retry_count+1})...\n")
    ...
    def _stage1_random_event(self, _=None, retry_count=0):
        self._log_debug(f"=== STAGE1: random_event (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"🎲 Этап 3/6: Проверка случайного события (попытка {retry_count+1})...\n")
    ...
    def _stage1_turn_order(self, _=None, retry_count=0):
        self._log_debug(f"=== STAGE1: turn_order (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"📋 Этап 4/6: Определение порядка NPC (попытка {retry_count+1})...\n")
    ...
    def _stage2_process_npc(self, _=None, retry_count=0):
    ...
    def _stage3_final(self, _=None, retry_count=0):
        self._log_debug(f"=== STAGE3: final narration (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"📖 Этап 5/6: Генерация финального ответа (попытка {retry_count+1})...\n")
    ...
    def _after_stage3_final(self, tool_calls, content, extra=None):
        ...
        try:
            ...
            self._log_debug("ERROR_IN_AFTER_STAGE3", str(e))
            import traceback
            self._log_debug("TRACEBACK", traceback.format_exc())
            # Аварийно завершаем генерацию, чтобы интерфейс не завис
            self.is_generating = False
            self.center_panel.set_input_state(tk.NORMAL)

    def _stage4_summary(self, _=None, retry_count=0):
        print("test4")
        self._log_debug(f"=== STAGE4: generating memory summary (attempt {retry_count+1}) ===")
        self.center_panel.display_system_message(f"📝 Этап 6/6: Сохранение краткой памяти (попытка {retry_count+1})...\n")
