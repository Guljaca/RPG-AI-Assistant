# Project_Py3_RPG_AI_main_Tools_version_V0.17.0.py (исправленный полностью)
...
import sys
from models import BaseObject, Narrator, Character, Location, Item, Event, Scenario, GameProfile
...
class MainApp(tk.Tk):
    ...
    def _handle_select_campaign(self, data):
        ...
        self._load_last_session()   # <-- баг 1 исправлен
    ...
    def _handle_create_campaign(self, data=None):
        ...
        if self.storage.create_campaign(name):
            ...
            messagebox.showinfo("Кампания", f"Кампания '{name}' создана.")
        else:
            messagebox.showerror("Ошибка", "Не удалось создать кампанию (возможно, уже существует).")
    ...
    def _handle_rename_campaign(self, data):
        ...
        if os.path.exists(new_path):
            messagebox.showerror("Ошибка", "Кампания с таким именем уже существует.")
            return
    ...
    def _handle_delete_campaign(self, data):
        ...
        if name == "Default":
            ...
            return
        if messagebox.askyesno("Удаление кампании", f"Удалить кампанию '{name}' и ВСЕ её данные? Это действие необратимо."):
    ...
    # ---------- Остальные методы (оставлены без изменений, кроме _handle_update_profile) ----------
    ...
    def _save_profile_to_file(self):
        profiles_dir = os.path.join(self.storage._get_campaign_path(), "profiles")
        os.makedirs(profiles_dir, exist_ok=True)
        filename = os.path.join(profiles_dir, f"{self.current_profile.name}.json")
        with open(filename, "w", encoding="utf-8") as f:
            json.dump(self.current_profile.to_dict(), f, ensure_ascii=False, indent=2)
    ...
    def _handle_update_object(self, obj_type: str, data: dict):
        ...
        if obj_id and obj_id in objects_dict:
            ...
            if old_name != name:
                self._delete_object_file_by_id(obj_type, obj_id)
                self.storage.save_object(obj_type, obj)
            action = "обновлён"
    ...
    def _save_current_session_safe(self):
        ...
        session_data = self._load_session_data(self.current_session_id)
        ...
        self._save_session_data(self.current_session_id, session_data)
    ...
    def _load_last_session(self):
        ...
        latest_time = None
        for sid in sessions:
            data = self._load_session_data(sid)
            if data is None:
                continue
    ...
    def _handle_update_profile(self, data):
        ...
        self._sync_profile_with_objects()
        self._save_current_session_safe()
        # Сохраняем профиль в файл (баг 4)
        self._save_profile_to_file()
        self._refresh_all_ui()
        self._cleanup_stage_prompts_narrators()
    ...
    def _handle_load_profile(self, data):
        ...
        if not name:
            return
        profiles_dir = os.path.join(self.storage._get_campaign_path(), "profiles")
        filename = os.path.join(profiles_dir, f"{name}.json")
        if not os.path.exists(filename):
            messagebox.showerror("Ошибка", f"Профиль '{name}' не найден.")
            return
        with open(filename, "r", encoding="utf-8") as f:
            profile_data = json.load(f)
        self.current_profile = GameProfile.from_dict(profile_data)
        self._sync_profile_with_objects()
        self._save_current_session_safe()
        self._refresh_all_ui()
        self._cleanup_stage_prompts_narrators()
...
class SettingsDialog:
    def __init__(self, parent, current_settings, stage_memory_config, stage_model_selection, stage_temperature_config):
        ...
        self.top.geometry("950x1200")
        self.top.transient(parent)
        self.top.grab_set()
        parent.update_idletasks()
        x = parent.winfo_rootx() + (parent.winfo_width() // 2) - (950 // 2)
        y = parent.winfo_rooty() + (parent.winfo_height() // 2) - (1200 // 2)
        ...
        ttk.Label(self.dual_frame, text="Основная модель (первая):").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        ...
        ttk.Label(self.dual_frame, text="Модель-переводчик (вторая):").grid(row=3, column=0, sticky="w", padx=5, pady=2)
        ...
        ttk.Label(main_frame, text="Макс. локаций на сцену:").grid(row=5, column=0, sticky="w", pady=5)
        ...
        self.max_locations.grid(row=5, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text="Макс. персонажей на сцену:").grid(row=6, column=0, sticky="w", pady=5)
        ...
        self.max_characters.grid(row=6, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text="Макс. предметов на сцену:").grid(row=7, column=0, sticky="w", pady=5)
        ...
        self.max_items.grid(row=7, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text="Макс. событий на сцену:").grid(row=8, column=0, sticky="w", pady=5)
        ...
        self.max_events.grid(row=8, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text="Макс. сценариев на сцену:").grid(row=9, column=0, sticky="w", pady=5)
        ...
        self.max_scenarios.grid(row=9, column=1, sticky="w", pady=5)

        self.enable_assoc_memory_var = tk.BooleanVar(value=current_settings.get("enable_associative_memory", True))
        ttk.Checkbutton(main_frame, text="Включить ассоциативную память", variable=self.enable_assoc_memory_var).grid(row=10, column=0, columnspan=2, sticky="w", pady=5)

        ttk.Label(main_frame, text="Макс. записей ассоциативной памяти на объект:").grid(row=11, column=0, sticky="w", pady=5)
        ...
        self.max_assoc_entries.grid(row=11, column=1, sticky="w", pady=5)

        stages_frame = ttk.LabelFrame(main_frame, text="Настройки этапов генерации")
        stages_frame.grid(row=12, column=0, columnspan=2, sticky="ew", pady=10)
        ...
        ttk.Label(stages_frame, text="Температура", font=('TkDefaultFont', 9, 'bold')).grid(row=0, column=6, padx=5, pady=2)
        ...
        for i, (key, label) in enumerate(stage_list, start=1):
            ...
            self._add_tooltip(temp_spin, "Оставьте пустым для использования стандартной температуры выбранной модели")
