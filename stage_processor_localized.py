...
class StageProcessor:
    ...
    def _handle_confirm_scene(self, obj_ids: list):
        ...
        if not self.stage_data.get("scene_narrative"):
            ...
            self._display_system(loc.tr("messages_auto_narrative", summary=summary))
    ...
    def _stage1_truth_check(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage2", attempt=retry_count+1))
    ...
    def _after_stage1_truth_check(self, content, extra):
        ...
        if not report_call:
            ...
            if retry_count < limit:
                self._display_error(loc.tr("messages_stage_retry", func="report_truth_check", attempt=retry_count+1, limit=limit))
                self._stage1_truth_check(retry_count+1)
                return
            else:
                self._display_system(loc.tr("messages_truth_skipped"))
        ...
        try:
            ...
            if edited:
                ...
                self._display_system(loc.tr("messages_truth_edited", edited=edited))
            else:
                self._display_system(loc.tr("messages_truth_no_violation"))
    ...
    def _stage1_player_action(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage3", attempt=retry_count+1))
        ...
        self._display_system(loc.tr("messages_dice_d20", value=dice_value))
    ...
    def _after_stage1_player_action(self, content, extra):
        ...
        if not tool_calls:
            ...
            if retry_count < limit:
                self._display_error(loc.tr("messages_stage_retry", func="act", attempt=retry_count+1, limit=limit))
                self._stage1_player_action(retry_count+1)
                return
            else:
                self.stage_data["player_action_desc"] = content.strip()[:500] if content else loc.tr("messages_default_action")
        ...
        try:
            ...
            self.stage_data["player_action_desc"] = description or loc.tr("messages_default_action")
            self._display_system(loc.tr("messages_action_result", desc=description))
            if self.debug_mode:
                output_items = {"player_action_desc": self.stage_data["player_action_desc"]}
                self._print_debug_section("Выходные данные этапа stage1_player_action", output_items, blank_lines_before=1, blank_lines_after=1)
            self._stage1_random_event_determine()
            ...
            if retry_count < limit:
                ...
                self.stage_data["player_action_desc"] = loc.tr("messages_default_action")
                self._stage1_random_event_determine()
                self._save_checkpoint("stage1_player_action")
    ...
    def _stage1_random_event_determine(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage4", attempt=retry_count+1))

        dice_value = self._pop_dice('d100')
        self.stage_data["event_occurrence_dice"] = dice_value
        self._display_system(loc.tr("messages_dice_d100", value=dice_value))
    ...
    def _after_stage1_random_event_determine(self, content, extra):
        ...
        if not tool_calls:
            ...
            if retry_count < limit:
                self._display_error(loc.tr("messages_stage_retry", func="report_random_event", attempt=retry_count+1, limit=limit))
        ...
        try:
            ...
            occurred_text = loc.tr("label_yes") if event_occurred else loc.tr("label_no")
            self._display_system(loc.tr("messages_event_occurred", occurred=occurred_text, dice=expected_dice))
    ...
    def _stage1_random_event_request_objects(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage5_1", attempt=retry_count+1))
    ...
    def _after_stage1_random_event_request_objects(self, content, extra):
        ...
        if send_call:
            try:
                ...
                if object_ids is not None and isinstance(object_ids, list) and object_ids:
                    self._display_system(loc.tr("messages_event_additional", oids=object_ids))
        ...
        self._display_system(loc.tr("messages_event_no_additional"))
        if self.debug_mode:
    ...
    def _stage1_random_event_details(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage5_2", attempt=retry_count+1))
        ...
        self._display_system(loc.tr("messages_dice_quality", value=quality_dice))
    ...
    def _after_stage1_random_event_details(self, content, extra):
        ...
        if report_call:
            try:
                ...
                self._display_system(loc.tr("messages_event_desc", desc=description))
                if self.debug_mode:
        ...
        if retry_count < limit:
            ...
            self.stage_data["event_desc"] = loc.tr("messages_default_event_desc")
            self._display_system(loc.tr("messages_auto_event"))
            if self.debug_mode:
    ...
    def _stage2_npc_action(self, retry_count=0):
        ...
        if not npc:
            self._display_error(loc.tr("messages_npc_skipped", npc_id=npc_id))
    ...
    def _after_stage2_npc_action(self, content, extra):
        ...
        self._display_system(loc.tr("messages_npc_action", name=npc_name, thought=thought, plan=plan))
    ...
    def _stage3_final(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage7", attempt=retry_count+1))
        ...
        event_ids = self.stage_data.get("scene_event_ids", [])
        events_text = ""
        if event_ids:
            event_descs = []
            for eid in event_ids:
                ev = self.main_app.events.get(eid)
                if ev:
                    event_descs.append(f"Событие: {ev.name}\n{ev.description}")
            if event_descs:
                events_text = "Активные события (могут произойти):\n" + "\n\n".join(event_descs)
        ...
        debug_inputs = {
            ...
            "events_text": events_text[:200] + "..." if len(events_text) > 200 else events_text
        }
    ...
    def _after_stage3_final(self, content, extra):
        ...
        if last_assistant_msg and final.strip() == last_assistant_msg.strip():
            self._display_error(loc.tr("messages_duplicate_response"))
        ...
        if last_user_msg and final.strip() == last_user_msg.strip():
            self._display_error(loc.tr("messages_duplicate_user"))
            limit = self._get_retry_limit("stage3_final")
            if retry_count < limit:
                self._stage3_final(retry_count+1)
                return
    ...
    def _stage11_validation(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage8_2", attempt=retry_count+1))
    ...
    def _after_stage11_validation(self, content, extra):
        ...
        if extracted is not None:
            if extracted.strip() == "":
                self._display_system(loc.tr("messages_validation_passed"))
                if self.debug_mode:
                ...
                if self._is_valid_narrative_text(corrected) and len(corrected) > 20:
                    ...
                    self.stage_data["final_response"] = corrected
                    self._display_system(loc.tr("messages_validation_corrected"))
                else:
        ...
        if cleaned and cleaned != content and len(cleaned) > 20:
            ...
            self._display_system(loc.tr("messages_validation_cleaned"))
            if self.debug_mode:
        ...
        if retry_count < max_retries:
            self._display_error(loc.tr("messages_stage_retry", func="validate_response", attempt=retry_count+1, limit=max_retries))
            self._stage11_validation(retry_count+1)
            return
        else:
    ...
    def _stage12_emotions(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage12", attempt=retry_count+1))
    ...
    def _process_next_emotion(self):
        if not self._emotion_queue:
            ...
            self._display_system(loc.tr("messages_emotions_done", emotions=self._emotion_results))
            if self.debug_mode:
            ...
            try:
                ...
                if isinstance(args, list):
                    ...
                    if args.get("id") == character_id:
                        ...
                        emotion = args.get("emotion", "Нейтрально")
                self._display_system(loc.tr("messages_emotion_for", char_id=character_id, emotion=emotion))
            except Exception as e:
    ...
    def _stage11_significant_changes(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage11", attempt=retry_count+1))
    ...
    def _finalize_significant_changes(self, significant: bool):
        ...
        self._display_system(loc.tr("messages_significant_yes") if significant else loc.tr("messages_significant_no"))
        self._stage4_summary()
    ...
    def _stage4_summary(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage9", attempt=retry_count+1))
    ...
    def _after_stage4_summary(self, content, extra):
        ...
        if summary and len(summary) > 10:
            ...
            self._display_system(loc.tr("messages_summary_added"))
            if self.debug_mode:
            ...
            self._display_system(loc.tr("messages_summary_failed"))
        self._stage10_associative_memory()
    ...
    def _stage10_associative_memory(self, retry_count=0):
        ...
        self._display_system(loc.tr("messages_stage10", attempt=retry_count+1))
    ...
    def _after_stage10_associative_memory(self, content, extra):
        ...
        if not changes:
            self._display_thinking(loc.tr("messages_assoc_failed"))
            self._finish_generation()
            return
    ...
    def _send_request(self, user_data: str, callback, extra=None, stage_name: str = None,
        ...
        if self.debug_mode and stage_name is not None:
            self._display_system(loc.tr("messages_debug_wait", stage=stage_name))
    ...
    def _do_send_request(self, user_data, callback, extra, stage_name, use_temp, show_in_thinking, context_data, model_choice, temp_override):
        ...
        if stage_name and stage_name in self.main_app.stage_prompts_config:
            ...
            for entry in reversed(config):
                if entry.startswith("narrator:"):
                    ...
                    mem_cfg = self.main_app.stage_memory_config.get(stage_name, {"max_history": 10, "max_summaries": 5})
        ...
        def stream_and_process():
            ...
            if error:
                ...
                self.main_app.after(0, lambda: self.main_app.center_panel.display_message(loc.tr("messages_model_error", error=error), "error"))
                ...
                return
            if self.main_app.stop_generation_flag:
                self.main_app.after(0, lambda: self.main_app.center_panel.display_message(loc.tr("messages_stopped"), "system"))
                self.main_app.after(0, lambda: setattr(self.main_app, 'is_generating', False))
                self.main_app.after(0, lambda: self.main_app.center_panel.set_input_state("normal"))
                return
    ...
    def _finish_generation(self):
        ...
        if total_time:
            self._display_system(loc.tr("messages_generation_completed", time=total_time))
