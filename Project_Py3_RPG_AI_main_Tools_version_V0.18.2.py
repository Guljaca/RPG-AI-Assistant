...
from visual_novel import VisualNovelFrame
...
class MainApp(tk.Tk):
    ...
    def _toggle_display_mode(self):
        if self.normal_frame.winfo_viewable():
            ...
            self.mode_switch_btn.config(text="Обычный режим")
            self.center_panel.display_system_message("🎮 Переключено в режим визуальной новеллы.\n")
            self.vn_frame.refresh_from_current_state()
        else:
    ...
    def _handle_delete_last_user_message(self, data=None):
        ...
        self.center_panel.update_translation_button_state()
        messagebox.showinfo("Удаление", "Последнее сообщение пользователя (и ответ ассистента, если был) удалено.")
    ...
    def _load_all_data(self):
        ...
        print("\n=== DEBUG _load_all_data: содержимое self.locations ===")
        for lid, loc in self.locations.items():
            print(f"ID: {lid}, name: {loc.name}, background_image: {getattr(loc, 'background_image', 'N/A')}")
        print("========================================\n")
    ...
    def _build_ui(self):
        ...
        ttk.Label(top_bar, text="Режим отображения:").pack(side=tk.LEFT, padx=5)
        self.mode_switch_btn = ttk.Button(top_bar, text="Визуальная новелла", command=self._toggle_display_mode)
