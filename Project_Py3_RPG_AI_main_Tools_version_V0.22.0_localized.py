# Project_Py3_RPG_AI_main_Tools_version_V0.20.0_localized.py
...
class MainApp(tk.Tk):
    ...
    def _handle_update_object(self, obj_type: str, data: dict):
        ...
        # Приведение associative_checks к строке
        ...
        if obj_id and obj_id in objects_dict:
            ...
            # Создание нового объекта (если id нет или не найден)
    ...
    def _handle_update_settings(self, data):
        ...
        # Новая настройка автоматического создания персонажей
...
class SettingsDialog:
    def __init__(self, parent, current_settings, stage_memory_config, stage_model_selection, stage_temperature_config):
        ...
        # НОВЫЙ ЧЕКБОКС ДЛЯ АВТОСОЗДАНИЯ ПЕРСОНАЖЕЙ
        self.enable_auto_char_var = tk.BooleanVar(value=current_settings.get("enable_auto_character_creation", True))
        ttk.Checkbutton(main_frame, text=loc.tr("settings_auto_character_creation"), 
                        variable=self.enable_auto_char_var).grid(row=6, column=0, columnspan=2, sticky="w", pady=5)

        ttk.Label(main_frame, text=loc.tr("settings_max_locations")).grid(row=7, column=0, sticky="w", pady=5)
        ...
        self.max_locations.grid(row=7, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text=loc.tr("settings_max_characters")).grid(row=8, column=0, sticky="w", pady=5)
        ...
        self.max_characters.grid(row=8, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text=loc.tr("settings_max_items")).grid(row=9, column=0, sticky="w", pady=5)
        ...
        self.max_items.grid(row=9, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text=loc.tr("settings_max_events")).grid(row=10, column=0, sticky="w", pady=5)
        ...
        self.max_events.grid(row=10, column=1, sticky="w", pady=5)

        ttk.Label(main_frame, text=loc.tr("settings_max_scenarios")).grid(row=11, column=0, sticky="w", pady=5)
        ...
        self.max_scenarios.grid(row=11, column=1, sticky="w", pady=5)

        self.enable_assoc_memory_var = tk.BooleanVar(value=current_settings.get("enable_associative_memory", True))
        ttk.Checkbutton(main_frame, text=loc.tr("settings_enable_assoc_memory"), variable=self.enable_assoc_memory_var).grid(row=12, column=0, columnspan=2, sticky="w", pady=5)

        ttk.Label(main_frame, text=loc.tr("settings_max_assoc_entries")).grid(row=13, column=0, sticky="w", pady=5)
        ...
        self.max_assoc_entries.grid(row=13, column=1, sticky="w", pady=5)

        stages_frame = ttk.LabelFrame(main_frame, text=loc.tr("settings_stage_config"))
        stages_frame.grid(row=14, column=0, columnspan=2, sticky="ew", pady=10)
    ...
    def _save(self):
        ...
        self.result = {
            ...
            "enable_auto_character_creation": self.enable_auto_char_var.get(),
        }
        self.top.destroy()
