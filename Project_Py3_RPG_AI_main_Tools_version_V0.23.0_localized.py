...
class MainApp(tk.Tk):
    def __init__(self):
        ...
        # Инициализация клиентов (разные URL)
        primary_api = self.settings.get("primary_api_url", self.settings.get("api_url", "http://localhost:1234/v1"))
        translator_api = self.settings.get("translator_api_url", self.settings.get("api_url", "http://localhost:1234/v1"))
        self.lm_client_primary = LMStudioClient(base_url=primary_api)
        self.lm_client_translator = LMStudioClient(base_url=translator_api)
        self.lm_client = self.lm_client_primary  # для обратной совместимости
    ...
    def _toggle_display_mode(self):
        if self.normal_frame.winfo_viewable():
            ...
            self.vn_frame.refresh_from_current_state()
            if hasattr(self.vn_frame, 'sync_translation'):
                self.vn_frame.sync_translation()
        else:
    ...
    def _handle_select_campaign(self, data):
        ...
        self.center_panel.chat_display.config(state=tk.NORMAL)
        self._load_last_session()
        self.center_panel.chat_display.see(tk.END)
        self.center_panel.chat_display.config(state=tk.DISABLED)
        # Оповещаем левую панель, что загрузка завершена
        if self.left_panel:
            self.left_panel._reset_loading()
    ...
    def _handle_update_object(self, obj_type: str, data: dict):
        ...
        if isinstance(assoc_checks_raw, list):
            assoc_checks = "\n".join(str(item) for item in assoc_checks_raw).strip()
        elif isinstance(assoc_checks_raw, str):
            assoc_checks = assoc_checks_raw.strip()
        else:
            assoc_checks = ""
        if not name:
        ...
        if obj_id and obj_id in objects_dict:
            ...
            self._handle_create_object(obj_type, data)
            return
        self._refresh_all_ui()
        self._save_current_session_safe()
    ...
    def _handle_delete_object(self, obj_type: str, obj_id: str):
        ...
        self._delete_object_image_files(obj_type, obj)

        # 2. Удаляем файл объекта через storage
        self.storage.delete_object(obj_type, obj_id)

        # 3. Удаляем из словаря
        del objects_dict[obj_id]

        # 4. Удаляем из профиля
        self._remove_from_profile(obj_type, obj_id)

        # 5. Удаляем из локальных описаний
        if obj_id in self.local_descriptions:
            del self.local_descriptions[obj_id]

        # 6. Удаляем из ассоциативной памяти
        if obj_id in self.associative_memory:
            del self.associative_memory[obj_id]

        # 7. Удаляем из stage_prompts_config (убираем narrator:...)
        if obj_type == "narrators":
            removed = False
            for stage, config in self.stage_prompts_config.items():
                new_config = [e for e in config if not (e.startswith("narrator:") and e[9:] == obj_id)]
                if len(new_config) != len(config):
                    self.stage_prompts_config[stage] = new_config
                    removed = True
            if removed:
                self.save_stage_prompts_config()
                # Обновить вкладку StagePromptsTab, если открыта
                if self.right_panel and self.right_panel.current_tab == "stage_prompts":
                    stage_frame = self.right_panel.tab_frames.get("stage_prompts")
                    if stage_frame and hasattr(stage_frame, "refresh"):
                        stage_frame.refresh()

        # 8. Сохраняем текущую сессию (профиль уже обновлён)
        self._save_current_session_safe()
    ...
    def _delete_object_image_files(self, obj_type: str, obj):
        """Удаляет все связанные с объектом изображения (аватары, спрайты, фоны)."""
        campaign_path = self.storage._get_campaign_path()
        if not campaign_path:
            return

        def delete_rel_path(rel_path: str):
            if not rel_path:
                return
            ...
            if os.path.exists(full):
                try:
                    ...
                    print(f"Не удалось удалить {full}: {e}")

        if obj_type == "characters":
            delete_rel_path(getattr(obj, 'avatar_image', ''))
            delete_rel_path(getattr(obj, 'sprite_image', ''))
            emotion_images = getattr(obj, 'emotion_images', {})
            for em_id, paths in emotion_images.items():
                delete_rel_path(paths.get('avatar', ''))
                delete_rel_path(paths.get('sprite', ''))
        elif obj_type == "locations":
            delete_rel_path(getattr(obj, 'background_image', ''))
        elif obj_type == "emotions":
            delete_rel_path(getattr(obj, 'avatar_image', ''))
            delete_rel_path(getattr(obj, 'sprite_image', ''))
    ...
    def load_settings(self) -> dict:
        default = {
            ...
            "primary_model": "",
            "translator_model": "",
            "primary_api_url": "http://localhost:1234/v1",
            "translator_api_url": "http://localhost:1234/v1",
            ...
            "language": "ru",
            "enable_auto_character_creation": True,
            "enabled_stages": {
                ...
                "stage13_auto_character_creator": True,
    ...
    def _save_current_session_safe(self):
        ...
        # ----- Очистка associative_memory от записей несуществующих объектов -----
        existing_ids = set(self.narrators.keys()) | set(self.characters.keys()) | set(self.locations.keys()) | \
                       set(self.items.keys()) | set(self.events.keys()) | set(self.scenarios.keys()) | set(self.emotions.keys())
        filtered_assoc = {oid: changes for oid, changes in self.associative_memory.items() if oid in existing_ids}
        if len(filtered_assoc) != len(self.associative_memory):
            self.associative_memory = filtered_assoc
            session_data["associative_memory"] = filtered_assoc
        # --------------------------------------------------------------------

        self.storage.save_session(self.current_session_id, session_data)

        # Fallback: если last_original_response пустой, но последний assistant есть — используем его как оригинал
        if not self.last_original_response and self.conversation_history:
            for msg in reversed(self.conversation_history):
                if msg["role"] == "assistant":
                    self.last_original_response = msg["content"]
    ...
    def _extract_tag_content(self, text: str, tag_name: str) -> str:
        """Извлекает содержимое тега <TagName>...</TagName> (регистронезависимо)."""
        pattern = re.compile(rf'<{re.escape(tag_name)}>(.*?)</{re.escape(tag_name)}>', re.DOTALL | re.IGNORECASE)
        match = pattern.search(text)
        if match:
            return match.group(1).strip()
        return ""

    def _parse_characters_from_description(self, description_text: str) -> List[dict]:
        """
        Извлекает персонажей из description, используя теги <Name>...</Name>.
        Возвращает список словарей с ключами name, description, short_description.
        Игнорирует служебные имена: Characters, Rules, Scenario, и т.п.
        """
        forbidden_names = {"characters", "rules", "scenario", "rule", "scenarios", "character"}
        characters = []
        # Ищем теги вида <Silerd> ... </Silerd>
        tag_pattern = re.compile(r'<([A-Za-z][A-Za-z0-9_]*)>\s*(.*?)\s*</\1>', re.DOTALL | re.IGNORECASE)
        matches = tag_pattern.findall(description_text)
        ...
        for ch in characters:
            if ch["name"].lower() not in seen:
                seen.add(ch["name"].lower())
                unique.append(ch)
        return unique

    def _import_character_card(self):
        """Импорт Character Card V3/V2: персонажи из тегов, сценарий и правила."""
        ...
        try:
            ...
            if "v3" in spec or spec == "chara_card_v3":
                data = card.get("data", {})
                card_name = data.get("name", "").strip()
                card_description = data.get("description", "")
                card_scenario_field = data.get("scenario", "")
                card_personality = data.get("personality", "")
                card_first_mes = data.get("first_mes", "")
            else:  # v2
                data = card.get("data", {})
                if not data and "data" in card:
                    data = card["data"]
                card_name = data.get("name", "").strip()
                card_description = data.get("description", "")
                card_scenario_field = data.get("scenario", "")
            ...
            if not card_name:
                card_name = "Без имени"
            # --- 1. Парсинг персонажей из тегов в description ---
            tag_chars = self._parse_characters_from_description(card_description)
            # Также проверим, нет ли персонажей в personality (иногда там бывают)
            if card_personality:
                tag_chars.extend(self._parse_characters_from_description(card_personality))
            # Удаляем дубликаты
            unique_chars = []
            seen_names = set()
            for ch in tag_chars:
            ...
            # --- 2. Импорт найденных персонажей ---
            imported_count = 0
            for ch_data in unique_chars:
                ch_name = ch_data["name"]
                # Проверка на существование
                exists = any(c.name.lower() == ch_name.lower() for c in self.characters.values())
                if exists:
                    continue
                new_char = Character(
                    name=ch_name,
                    description=ch_data["description"],
                    short_description=ch_data.get("short_description", ch_data["description"][:200]),
                    associative_checks=data.get("post_history_instructions", ""),
                    is_player=False
                )
                ...
                imported_count += 1
            if imported_count > 0:
                messagebox.showinfo(loc.tr("import_success"), f"Импортировано персонажей: {imported_count}")
            elif not tag_chars:
                # Если нет теговых персонажей, возможно, карточка описывает одного персонажа
                # Тогда импортируем имя карточки как персонажа
                existing = any(c.name.lower() == card_name.lower() for c in self.characters.values())
                if not existing:
                    full_desc = card_description
                    if card_personality:
                        full_desc += "\n\n--- Характер ---\n" + card_personality
                    if not full_desc.strip():
                        full_desc = "Нет описания."
                    new_char = Character(
                        name=card_name,
                        description=full_desc,
                        short_description=full_desc[:200] + "…" if len(full_desc) > 200 else full_desc,
                        associative_checks=data.get("post_history_instructions", ""),
                        is_player=False
                    )
                    self.storage.save_object("characters", new_char)
                    ...
                    if new_char.id not in self.current_profile.enabled_characters:
                        self.current_profile.enabled_characters.append(new_char.id)
                        self._save_profile_to_file()
                    messagebox.showinfo(loc.tr("import_success"), f"Импортирован персонаж '{card_name}'")
            # --- 3. Импорт сценария ---
            # Сначала пытаемся взять из тега <Scenario> в description
            scenario_text = self._extract_tag_content(card_description, "Scenario")
            if not scenario_text:
                # Если нет, используем поле scenario из карточки
                scenario_text = card_scenario_field.strip()
            rules_text = self._extract_tag_content(card_description, "Rules")
            # Объединяем Scenario и Rules, если есть
            final_scenario = scenario_text
            if rules_text:
                final_scenario += "\n\n--- Правила (Rules) ---\n" + rules_text
            if final_scenario and len(final_scenario) > 10:
                # Проверка на существование сценария с таким же текстом
                existing_scenario = None
                for sc in self.scenarios.values():
                    if sc.description == final_scenario:
                        existing_scenario = sc
                        break
                if not existing_scenario:
                    scenario_name = f"{card_name} — основная сюжетная линия"
                    short_scenario = final_scenario[:200] + "…" if len(final_scenario) > 200 else final_scenario
                    new_scenario = Scenario(
                        name=scenario_name,
                        description=final_scenario,
                        short_description=short_scenario,
                        associative_checks=""
                    )
            ...
            # --- 4. Импорт аватара (если PNG) для первого импортированного персонажа ---
            if is_png and tag_chars:
                first_char_name = tag_chars[0]["name"]
                first_char = None
                for c in self.characters.values():
                    if c.name.lower() == first_char_name.lower():
                        first_char = c
                        break
                if first_char:
                    avatar_rel = self._copy_image_to_campaign(file_path, "characters/avatars")
                    first_char.avatar_image = avatar_rel
                    self.storage.save_object("characters", first_char)
                    self.characters[first_char.id] = first_char
                    messagebox.showinfo(loc.tr("import_success"), f"Аватар для '{first_char.name}' установлен.")
            # --- 5. Обновление UI ---
            self._refresh_all_ui()
            if self.right_panel:
                self.right_panel.show_tab("characters")
                if tag_chars:
                    first_char = next((c for c in self.characters.values() if c.name.lower() == tag_chars[0]["name"].lower()), None)
                    if first_char:
                        char_tab = self.right_panel.tab_frames.get("characters")
                        if char_tab and hasattr(char_tab, "_select_object_by_id"):
                            char_tab._select_object_by_id(first_char.id)
        except Exception as e:
    ...
    def _handle_load_session(self, data):
        ...
        def do_load():
            ...
            self.left_panel.refresh_session_list()
            self.center_panel.clear_chat()
            self.center_panel.bulk_display_history(self.conversation_history)
            ...
            session_data["last_used"] = datetime.now().isoformat()
            self.storage.save_session(session_id, session_data)
            # Сброс флага загрузки левой панели
            if self.left_panel:
                self.left_panel._reset_loading()
    ...
    def _auto_translate_if_enabled(self):
        """Переводит ТОЛЬКО финальный рассказ (последний assistant в истории)."""
        final_text = ""
        for msg in reversed(self.conversation_history):
            if msg["role"] == "assistant":
        ...
        if (self.enable_assistant_translation 
            and self.use_two_models 
            and len(final_text) > 150
            and final_text.lower() not in ("ничего", "none", "null", "пусто", "")):

            self.last_original_response = final_text
            if self.center_panel:
                self.center_panel.update_translation_button_state()

            self.after(500, lambda: self._translate_response_stream(final_text, response_start_index=None))
    ...
    def _handle_update_settings(self, data):
        ...
        self.enable_auto_character_creation = data.get("enable_auto_character_creation", True)
        self.settings["enable_auto_character_creation"] = self.enable_auto_character_creation
        ...
        primary_url = data.get("primary_api_url", data.get("api_url", "http://localhost:1234/v1"))
        translator_url = data.get("translator_api_url", data.get("api_url", "http://localhost:1234/v1"))
        self.lm_client_primary.base_url = primary_url
        self.lm_client_translator.base_url = translator_url
    ...
    def _translate_response_stream(self, original_content: str, response_start_index: str = None):
        text = (original_content or "").strip()
        if len(text) < 15 or text.lower() in ("ничего", "none", "null", "пусто", ""):
            return
        ...
        def translate_stream():
            ...
            thinking_buffer = ""
            try:
                for chunk in self.lm_client_translator.chat_completion_stream(messages=messages, model=model, temperature=temp, max_tokens=max_tok):
                    if self.stop_generation_flag:
                        break
                    if chunk["type"] == "reasoning":
                        thinking_buffer += chunk["text"]
                        ...
                        self.after(0, lambda t=chunk["text"]: self.center_panel.append_translation_stream(t))
                    elif chunk["type"] == "error":
...
class SettingsDialog:
    def __init__(self, parent, current_settings, stage_memory_config, stage_model_selection, stage_temperature_config):
        ...
        # Single mode frame (без Temperature)
        ...
        ttk.Label(self.single_frame, text=loc.tr("settings_max_tokens")).grid(row=1, column=0, sticky="w", padx=5, pady=2)
        ...
        self.single_max_tokens.grid(row=1, column=1, sticky="w", padx=5, pady=2)
        self.single_frame.columnconfigure(1, weight=1)

        # Dual mode frame (без Temperature)
        ...
        ttk.Label(self.dual_frame, text=loc.tr("settings_primary_max_tokens")).grid(row=1, column=0, sticky="w", padx=5, pady=2)
        ...
        self.primary_max_tokens.grid(row=1, column=1, sticky="w", padx=5, pady=2)
        ttk.Label(self.dual_frame, text=loc.tr("settings_translator_model")).grid(row=2, column=0, sticky="w", padx=5, pady=2)
        self.translator_model_combo = ttk.Combobox(self.dual_frame, width=40)
        self.translator_model_combo.grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        self.translator_model_combo.set(current_settings.get("translator_model", "local-model"))
        ttk.Label(self.dual_frame, text=loc.tr("settings_translator_max_tokens")).grid(row=3, column=0, sticky="w", padx=5, pady=2)
        ...
        self.translator_max_tokens.grid(row=3, column=1, sticky="w", padx=5, pady=2)

        # API URL для двух моделей
        ttk.Label(self.dual_frame, text="API URL (основная модель):").grid(row=4, column=0, sticky="w", padx=5, pady=2)
        self.primary_api_url_entry = ttk.Entry(self.dual_frame, width=40)
        self.primary_api_url_entry.grid(row=4, column=1, sticky="ew", padx=5, pady=2)
        self.primary_api_url_entry.insert(0, current_settings.get("primary_api_url", current_settings.get("api_url", "http://localhost:1234/v1")))

        ttk.Label(self.dual_frame, text="API URL (модель-переводчик):").grid(row=5, column=0, sticky="w", padx=5, pady=2)
        self.translator_api_url_entry = ttk.Entry(self.dual_frame, width=40)
        self.translator_api_url_entry.grid(row=5, column=1, sticky="ew", padx=5, pady=2)
        self.translator_api_url_entry.insert(0, current_settings.get("translator_api_url", current_settings.get("api_url", "http://localhost:1234/v1")))
        ...
        # Максимальные количества объектов в сцене
        ...
        # Таблица конфигурации этапов
        stages_frame = ttk.LabelFrame(main_frame, text=loc.tr("settings_stage_config"))
        stages_frame.grid(row=11, column=0, columnspan=2, sticky="ew", pady=10)
        ...
        stage_list = [
            ...
            ("stage13_auto_character_creator", "13. Автосоздание персонажей"),
    ...
    def _refresh_models_list(self):
        ...
        def _fetch_models(api_url: str) -> list:
            if not api_url:
                return ["local-model"]
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    messagebox.showwarning(loc.tr("settings_refresh_models"), f"Не удалось получить список моделей с {api_url} (HTTP {response.status_code})")
            except Exception as e:
                messagebox.showerror(loc.tr("error_invalid_name"), f"Ошибка при запросе к {api_url}:\n{e}")
            return models

        # Single mode
        if not use_two:
            api_url = self.api_url.get().strip()
            models = _fetch_models(api_url)
        ...
        # Dual mode - separate requests!
        primary_url = self.primary_api_url_entry.get().strip()
        translator_url = self.translator_api_url_entry.get().strip()

        primary_models = _fetch_models(primary_url)
        translator_models = _fetch_models(translator_url)

        self.primary_model_combo['values'] = primary_models
        self.translator_model_combo['values'] = translator_models

        if self.primary_model_combo.get() not in primary_models:
            self.primary_model_combo.set(primary_models[0] if primary_models else "local-model")
        if self.translator_model_combo.get() not in translator_models:
            self.translator_model_combo.set(translator_models[0] if translator_models else "local-model")

        # Also update single combo just in case
        self.model_name_combo['values'] = primary_models
    ...
    def _toggle_two_models(self):
        if self.use_two_models_var.get():
            ...
            self.primary_api_url_entry.config(state="normal")
            self.translator_api_url_entry.config(state="normal")
            ...
            self.primary_api_url_entry.config(state="disabled")
            self.translator_api_url_entry.config(state="disabled")
            for widget in self.model_choice_widgets.values():
    ...
    def _save(self):
        ...
        self.result = {
            ...
            "primary_api_url": self.primary_api_url_entry.get().strip(),
            "translator_api_url": self.translator_api_url_entry.get().strip(),
            "max_tokens": int(self.single_max_tokens.get()),
            "enable_memory_summary": True,
            "max_memory_summaries": 5,
            "enable_associative_memory": True,      # всегда включена (управляется стадией)
            "max_associative_memory_entries": 5,    # значение по умолчанию
            ...
            "enable_auto_character_creation": True,   # управляется стадией
        }
        self.top.destroy()
