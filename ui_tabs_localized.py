...
class AvatarCropEditor(tk.Toplevel):
    def __init__(self, parent, sprite_path, callback):
        ...
        self.title(loc.tr("vn_crop_title"))
    ...
    def _load_image(self):
        if not self.sprite_path or not os.path.exists(self.sprite_path):
            messagebox.showerror(loc.tr("error_invalid_name"), loc.tr("error_file_not_found"), parent=self)
            self.destroy()
            return
        try:
            self.original_image = Image.open(self.sprite_path).convert("RGBA")
        except Exception as e:
            messagebox.showerror(loc.tr("error_invalid_name"), loc.tr("error_image_load", error=str(e)), parent=self)
            self.destroy()
    ...
    def _build_ui(self):
        ...
        ttk.Label(bottom_frame, text=loc.tr("vn_crop_size_label")).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(bottom_frame, text=loc.tr("vn_crop_apply_size"), command=self._apply_size).pack(side=tk.LEFT, padx=5)
        ttk.Button(bottom_frame, text=loc.tr("vn_crop_save"), command=self._save_crop).pack(side=tk.LEFT, padx=5)
        ttk.Button(bottom_frame, text=loc.tr("vn_crop_abort"), command=self.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def _update_info(self):
        ...
        self.info_label.config(text=loc.tr("vn_crop_info", w=w, h=h, size=self.crop_size))
    ...
    def _save_crop(self):
        ...
        if right <= left or bottom <= top:
            messagebox.showerror(loc.tr("error_invalid_name"), loc.tr("error_crop_invalid"), parent=self)
            return
...
class ProfileTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(row0, text=loc.tr("profile_label")).pack(side=tk.LEFT)
        ...
        ttk.Button(row1, text=loc.tr("profile_load"), command=self._load_profile).pack(side=tk.LEFT, padx=2)
        ttk.Button(row1, text=loc.tr("profile_save"), command=self._save_profile).pack(side=tk.LEFT, padx=2)
        ttk.Button(row1, text=loc.tr("profile_new"), command=self._new_profile).pack(side=tk.LEFT, padx=2)
        ...
        bottom_frame = ttk.Frame(self)
        bottom_frame.pack(fill=tk.X, padx=5, pady=5)
        self.apply_btn = ttk.Button(bottom_frame, text=loc.tr("profile_apply"), command=self._apply_changes)
    ...
    def refresh(self):
        ...
        if self.app.narrators:
            ttk.Label(self.scrollable_frame, text=loc.tr("profile_narrators"), font=('Arial', 10, 'bold')).pack(anchor='w', pady=(5,0))
        ...
        if self.app.characters:
            ttk.Label(self.scrollable_frame, text=loc.tr("profile_characters"), font=('Arial', 10, 'bold')).pack(anchor='w', pady=(10,0))
            ...
            for cid, char in self.app.characters.items():
                ...
                player_tag = f" {loc.tr('editor_is_player')}" if char.is_player else ""
                cb = ttk.Checkbutton(self.scrollable_frame, text=f"{char.name}{player_tag}", variable=var, command=self._on_checkbox_change)
                cb.pack(anchor='w', padx=20)

        if self.app.locations:
            ttk.Label(self.scrollable_frame, text=loc.tr("profile_locations"), font=('Arial', 10, 'bold')).pack(anchor='w', pady=(10,0))
            self.loc_vars.clear()
            for lid, loc_obj in self.app.locations.items():
                var = tk.BooleanVar(value=(lid in profile.enabled_locations))
                self.loc_vars[lid] = var
                cb = ttk.Checkbutton(self.scrollable_frame, text=loc_obj.name, variable=var, command=self._on_checkbox_change)
                cb.pack(anchor='w', padx=20)

        if self.app.items:
            ttk.Label(self.scrollable_frame, text=loc.tr("profile_items"), font=('Arial', 10, 'bold')).pack(anchor='w', pady=(10,0))
        ...
        if self.app.events:
            ttk.Label(self.scrollable_frame, text=loc.tr("profile_events"), font=('Arial', 10, 'bold')).pack(anchor='w', pady=(10,0))
        ...
        if self.app.scenarios:
            ttk.Label(self.scrollable_frame, text=loc.tr("profile_scenarios"), font=('Arial', 10, 'bold')).pack(anchor='w', pady=(10,0))
        ...
        if self.app.emotions:
            ttk.Label(self.scrollable_frame, text=loc.tr("profile_emotions"), font=('Arial', 10, 'bold')).pack(anchor='w', pady=(10,0))
    ...
    def _apply_changes(self):
        ...
        messagebox.showinfo(loc.tr("profile_tab"), loc.tr("profile_apply"))
    ...
    def _save_profile(self):
        ...
        if not name or name == "Default":
            new_name = simpledialog.askstring(loc.tr("profile_save"), loc.tr("profile_new"), initialvalue=name, parent=self)
            if not new_name:
                return
...
class BaseEditorTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        list_frame = ttk.LabelFrame(scrollable_frame, text=loc.tr("editor_title_list", title=self.title))
        ...
        ttk.Button(btn_frame, text=loc.tr("editor_create"), command=self._create_new).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text=loc.tr("editor_delete"), command=self._delete_selected).pack(side=tk.LEFT, padx=5)

        mode_frame = ttk.LabelFrame(scrollable_frame, text=loc.tr("editor_mode_global"))
        mode_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Radiobutton(mode_frame, text=loc.tr("editor_mode_global"), variable=self.editing_mode, value="global", command=self._on_mode_change).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(mode_frame, text=loc.tr("editor_mode_local"), variable=self.editing_mode, value="local", command=self._on_mode_change).pack(side=tk.LEFT, padx=5)
        ...
        if self.obj_type == "characters":
            ...
            ttk.Label(avatar_row, text=loc.tr("editor_avatar_neutral")).grid(row=0, column=0, sticky="w", padx=5)
            ...
            ttk.Label(sprite_row, text=loc.tr("editor_sprite_neutral")).grid(row=0, column=0, sticky="w", padx=5)
            ...
            emotion_frame = ttk.LabelFrame(scrollable_frame, text=loc.tr("editor_images_by_emotion"))
        ...
        if self.obj_type == "locations":
            bg_frame = ttk.LabelFrame(scrollable_frame, text=loc.tr("editor_images_vn"))
            ...
            ttk.Label(bg_row, text=loc.tr("editor_background")).grid(row=0, column=0, sticky="w", padx=5)
        ...
        if self.obj_type == "emotions":
            emotion_images_frame = ttk.LabelFrame(scrollable_frame, text=loc.tr("editor_emotion_images"))
            ...
            ttk.Label(avatar_row, text=loc.tr("editor_emotion_avatar")).grid(row=0, column=0, sticky="w", padx=5)
            ...
            ttk.Label(sprite_row, text=loc.tr("editor_emotion_sprite")).grid(row=0, column=0, sticky="w", padx=5)
        ...
        self.save_btn = ttk.Button(bottom_frame, text=loc.tr("editor_save"), command=self._save_current)
    ...
    def _select_avatar(self):
        filepath = filedialog.askopenfilename(
            title=loc.tr("editor_avatar_neutral"),
    ...
    def _crop_avatar_from_sprite(self):
        ...
        if not sprite_path:
            messagebox.showwarning(loc.tr("vn_crop_title"), loc.tr("warning_crop_no_sprite"))
            return
        full_sprite_path = self._get_full_path(sprite_path)
        if not os.path.exists(full_sprite_path):
            messagebox.showerror(loc.tr("error_invalid_name"), loc.tr("error_sprite_not_found"))
            return
    ...
    def _select_sprite(self):
        filepath = filedialog.askopenfilename(
            title=loc.tr("editor_sprite_neutral"),
            filetypes=[("Image files", "*.png *.jpg *.jpeg *.gif *.bmp")]
        )
        if filepath:
    ...
    def _select_background(self):
        filepath = filedialog.askopenfilename(
            title=loc.tr("editor_background"),
            filetypes=[("Image files", "*.png *.jpg *.jpeg *.gif *.bmp")]
        )
        if filepath:
    ...
    def _rebuild_emotion_ui(self):
        ...
        if not hasattr(self.app, 'emotions') or not self.app.emotions:
            ttk.Label(self.emotion_container, text=loc.tr("info_no_emotions")).pack(pady=5)
            return
        ...
        for em_id, em in self.app.emotions.items():
            ...
            ttk.Label(avatar_frame, text=loc.tr("editor_emotion_avatar")).grid(row=0, column=0, sticky="w", padx=5)
            ...
            ttk.Label(sprite_frame, text=loc.tr("editor_emotion_sprite")).grid(row=0, column=0, sticky="w", padx=5)
    ...
    def _select_emotion_avatar(self, emotion_id, var):
        filepath = filedialog.askopenfilename(
            title=loc.tr("editor_emotion_avatar"),
            filetypes=[("Image files", "*.png *.jpg *.jpeg *.gif *.bmp")]
        )
        if filepath:
    ...
    def _select_emotion_sprite(self, emotion_id, var):
        filepath = filedialog.askopenfilename(
            title=loc.tr("editor_emotion_sprite"),
            filetypes=[("Image files", "*.png *.jpg *.jpeg *.gif *.bmp")]
        )
        if filepath:
            rel_path = self._copy_image_to_campaign(filepath, f"characters/emotions/{emotion_id}")
    ...
    def _populate_list(self):
        ...
        for obj_id in sorted_ids:
            ...
            if self.obj_type == "characters" and obj.is_player:
                text += f" [{loc.tr('editor_is_player')}]"
    ...
    def _reset_local(self):
        if self.current_obj_id and self.current_obj_id in self.app.local_descriptions:
            if messagebox.askyesno(loc.tr("editor_reset_local"), loc.tr("confirm_reset_local")):
...
class SystemPromptsTab(ttk.Frame):
    ...
    def _build_ui(self):
        top_frame = ttk.LabelFrame(self, text=loc.tr("prompts_list"))
        ...
        ttk.Button(btn_frame, text=loc.tr("prompts_create"), command=self._create_new).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text=loc.tr("prompts_delete"), command=self._delete_selected).pack(side=tk.LEFT, padx=5)

        editor_frame = ttk.LabelFrame(self, text=loc.tr("prompts_editor"))
        editor_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.name_label = ttk.Label(editor_frame, text=loc.tr("prompts_name"))
        ...
        self.name_entry.pack(fill=tk.X, padx=5, pady=2)
        ttk.Label(editor_frame, text=loc.tr("prompts_content")).pack(anchor='w', padx=5, pady=2)
        ...
        self.save_btn = ttk.Button(btn_frame2, text=loc.tr("prompts_save"), command=self._save_current)
    ...
    def _save_current(self):
        ...
        messagebox.showinfo(loc.tr("prompts_save"), loc.tr("prompts_save", name=self.current_prompt_name))
    ...
    def _create_new(self):
        name = simpledialog.askstring(loc.tr("prompts_create"), loc.tr("prompts_create"), parent=self)
        if name:
            if name.startswith("translator_"):
                messagebox.showwarning(loc.tr("error_invalid_name"), loc.tr("error_invalid_name"))
                return
...
class TranslatorPromptsTab(ttk.Frame):
    ...
    def _build_ui(self):
        top_frame = ttk.LabelFrame(self, text=loc.tr("translator_prompts_list"))
        top_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.listbox = tk.Listbox(top_frame, height=8)
        scrollbar = ttk.Scrollbar(top_frame, orient=tk.VERTICAL, command=self.listbox.yview)
        self.listbox.configure(yscrollcommand=scrollbar.set)
...
class StagePromptsTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        info_frame = ttk.LabelFrame(self, text=loc.tr("stage_info"))
        info_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        info_text = loc.tr("stage_info_text")
        ...
        left_frame = ttk.LabelFrame(main_row, text=loc.tr("stage_list_header"))
        ...
        right_frame = ttk.LabelFrame(main_row, text=loc.tr("stage_prompts_header"))
        ...
        ttk.Button(btn_frame, text=loc.tr("stage_add_prompt"), command=self._add_prompt).grid(row=0, column=0, padx=3, pady=2, sticky="ew")
        ttk.Button(btn_frame, text=loc.tr("stage_add_narrators"), command=self._add_narrators).grid(row=0, column=1, padx=3, pady=2, sticky="ew")
        ttk.Button(btn_frame, text=loc.tr("stage_remove_selected"), command=self._remove_prompt).grid(row=1, column=0, padx=3, pady=2, sticky="ew")
        ttk.Button(btn_frame, text=loc.tr("stage_remove_all_narrators"), command=self._remove_all_narrators).grid(row=1, column=1, padx=3, pady=2, sticky="ew")
        ttk.Button(btn_frame, text=loc.tr("stage_move_up"), command=lambda: self._move_prompt(-1)).grid(row=2, column=0, padx=3, pady=2, sticky="ew")
        ttk.Button(btn_frame, text=loc.tr("stage_move_down"), command=lambda: self._move_prompt(1)).grid(row=2, column=1, padx=3, pady=2, sticky="ew")
        ttk.Button(btn_frame, text=loc.tr("stage_add_history"), command=self._add_history).grid(row=3, column=0, padx=3, pady=2, sticky="ew")
        ttk.Button(btn_frame, text=loc.tr("stage_save"), command=self._save_config).grid(row=3, column=1, padx=3, pady=2, sticky="ew")
    ...
    def _add_prompt(self):
        if not self.current_stage:
            messagebox.showwarning(loc.tr("error_invalid_name"), loc.tr("stage_add_prompt"))
            return
        all_prompts = self.app.prompt_manager.list_prompts()
        all_prompts = [p for p in all_prompts if not p.startswith("translator_")]
        if not all_prompts:
            messagebox.showinfo(loc.tr("prompts_list"), loc.tr("warning_no_objects"))
            return

        dialog = tk.Toplevel(self)
        dialog.title(loc.tr("stage_add_prompt"))
        dialog.geometry("400x300")
        dialog.transient(self)
        center_window(dialog, self.app)

        ttk.Label(dialog, text=loc.tr("prompts_list")).pack(pady=5)
        ...
        ttk.Button(dialog, text=loc.tr("profile_apply"), command=on_select).pack(pady=5)
        ttk.Button(dialog, text=loc.tr("vn_cancel"), command=dialog.destroy).pack(pady=2)
        listbox.bind("<Double-Button-1>", lambda e: on_select())

    def _add_narrators(self):
        if not self.current_stage:
            messagebox.showwarning(loc.tr("error_invalid_name"), loc.tr("stage_add_narrators"))
            return

        narrators = self.app.narrators
        if not narrators:
            messagebox.showinfo(loc.tr("profile_narrators"), loc.tr("warning_no_narrators"))
            return

        dialog = tk.Toplevel(self)
        dialog.title(loc.tr("stage_add_narrators"))
        ...
        ttk.Label(frame, text=loc.tr("profile_narrators")).pack(anchor='w')
        ...
        def add_selected():
            ...
            if not selected_indices:
                messagebox.showwarning(loc.tr("stage_add_narrators"), loc.tr("error_invalid_name"))
                return
            current = self.app.stage_prompts_config.get(self.current_stage, [])
            ...
            if added:
                ...
                self._refresh_prompts_list()
                self.app.save_stage_prompts_config()
                messagebox.showinfo(loc.tr("stage_add_narrators"), loc.tr("stage_add_narrators"))
            else:
                messagebox.showinfo(loc.tr("stage_add_narrators"), loc.tr("error_invalid_name"))
            dialog.destroy()

        btn_frame = ttk.Frame(dialog)
        btn_frame.pack(fill=tk.X, pady=5)
        ttk.Button(btn_frame, text=loc.tr("stage_add_narrators"), command=add_selected).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text=loc.tr("vn_cancel"), command=dialog.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def _remove_all_narrators(self):
        ...
        if len(new_list) == len(current):
            messagebox.showinfo(loc.tr("stage_remove_all_narrators"), loc.tr("error_invalid_name"))
            return
        self.app.stage_prompts_config[self.current_stage] = new_list
        self._refresh_prompts_list()
        self.app.save_stage_prompts_config()
        messagebox.showinfo(loc.tr("stage_remove_all_narrators"), loc.tr("stage_remove_all_narrators"))
    ...
    def _save_config(self):
        self.app.save_stage_prompts_config()
        messagebox.showinfo(loc.tr("stage_save"), loc.tr("stage_save"))
    ...
    def _add_history(self):
        if not self.current_stage:
            messagebox.showwarning(loc.tr("error_invalid_name"), loc.tr("stage_add_history"))
            return
        current = self.app.stage_prompts_config.get(self.current_stage, [])
        if "history:auto" in current:
            messagebox.showinfo(loc.tr("stage_add_history"), loc.tr("info_history_already"))
            return
        ...
        self.app.save_stage_prompts_config()
        messagebox.showinfo(loc.tr("stage_add_history"), loc.tr("info_history_added"))
    ...
    def _refresh_prompts_list(self):
        ...
        for entry in prompts:
            if entry.startswith("narrator:"):
                ...
                display = "📜 " + loc.tr("stage_add_history")
            else:
                display = f"💬 {entry}"
...
class HistoryTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        self.tree.heading("num", text=loc.tr("history_column_num"))
        self.tree.heading("user_msg", text=loc.tr("history_column_user"))
        self.tree.heading("assistant_msg", text=loc.tr("history_column_assistant"))
        self.tree.heading("importance", text=loc.tr("history_column_importance"))
        ...
        info_frame = ttk.LabelFrame(self, text=loc.tr("history_tab"))
        info_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Label(info_frame, text=loc.tr("history_help"), justify=tk.LEFT).pack(padx=5, pady=5)
    ...
    def refresh(self):
        ...
        for idx, ((user_msg, asst_msg), flag) in enumerate(zip(pairs, flags), start=1):
            ...
            flag_str = loc.tr("label_true") if flag else loc.tr("label_false")
    ...
    def _on_click_importance(self, event):
        ...
        self.app._save_current_session_safe()
        self.refresh()
        self.app.center_panel.display_system_message(loc.tr("history_help") + "\n")
    ...
    def _on_double_click(self, event):
        ...
        win.title(loc.tr("history_edit_title", idx=idx+1))
        win.geometry("700x500")
        win.transient(self)
        win.grab_set()
        center_window(win, self.app)

        ttk.Label(win, text=loc.tr("history_user_label"), font=('TkDefaultFont', 10, 'bold')).pack(anchor='w', padx=10, pady=(10,0))
        ...
        ttk.Label(win, text=loc.tr("history_assistant_label"), font=('TkDefaultFont', 10, 'bold')).pack(anchor='w', padx=10)
        ...
        flag_cb = ttk.Checkbutton(win, text=loc.tr("history_important_check"), variable=flag_var)
        ...
        ttk.Button(btn_frame, text=loc.tr("history_save"), command=save_flag).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text=loc.tr("vn_cancel"), command=win.destroy).pack(side=tk.LEFT, padx=10)